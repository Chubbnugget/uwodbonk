---
id: 3966
category: Medium Sized Creature
---
# Discovery: albino raccoon

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Medium Sized Creature |
| **Discovery Name** | albino raccoon |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A raccoon whose whole body is pure white. Normally, they have grayish-brown fur, but due to a genetic mutation, individuals with white fur and reddish eyes and noses are very rarely born. Their downy fur is very mysterious. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 14 (Rank of skills required for discovery) |
| **Experience** | 1230 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Discovery Name Card:615 |
| **最終更新** | 2025-06-27 17:30:52 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional raccoon investigation](docs/Quests/Other/6001-7000/quest_6957.md)** | Pursuit quest (9) | Ecological research (12) Biology (14) English (1) | Reward: 350,000 / Advance payment: 0Experience: 860 |