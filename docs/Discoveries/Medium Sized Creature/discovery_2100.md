---
id: 2100
category: Medium Sized Creature
---
# Discovery: indri

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Medium Sized Creature |
| **Discovery Name** | indri |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | It sings so loud that it can be heard for several kilometers in the forests of Madagascar. The voice seems to be a means of communicating with friends. It eats young leaves, fruits, and flowers. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 630 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | forest chorus |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Forest Chorus](docs/Quests/Adventure/1-1000/quest_684.md)** | Adventure quest (7) | Ecological research (6) Biology (8) Swahili | Reward: 128,000 / Advance payment: 0Experience: 255 / Fame: 90 |