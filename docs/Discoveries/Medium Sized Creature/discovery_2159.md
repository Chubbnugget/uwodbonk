---
id: 2159
category: Medium Sized Creature
---
# Discovery: babilusa

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Medium Sized Creature |
| **Discovery Name** | babilusa |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | Babirusa means "pig, deer" in the local language. It is characterized by its fangs that pierce the skin from its upper jaw. The tusks are used in fights between males. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 4 (Rank of skills required for discovery) |
| **Experience** | 220 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Biology like a log

 Discovery Experience 220 Discovery Name Card Experience 110 |
| **最終更新** | 2016-07-26 12:31:13 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Log-like Biology](Quests/Adventure/quest_1298.md)** | Adventure quest (3) | Ecological Survey (2) Biology (4) Malayu Tagalog | Reward: 78,000 / Advance payment: 0Experience: 205 / Fame: 130 |