---
id: 3780
category: Medium Sized Creature
---
# Discovery: Formosan wildcat

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Medium Sized Creature |
| **Discovery Name** | Formosan wildcat |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | The wildcat is considered to be endemic to the island of Taiwan. The tips of the ears are rounded and there is a white spot on the back. It is very close in species to the Japanese Iriomote wildcat. When it catches prey, it has the habit of weakening it by hitting it with its front legs. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 1040 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Card experience: 520 |
| **最終更新** | 2023-03-23 ​​08:49:47 Seraphine |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Cat hiding in the mountain forest of the island](Quests/Adventure/quest_6738.md)** | Adventure quest (8) | Ecological Survey (9) Biology (11) Malayu Tagalog (1) | Reward: 380,000/ Advance: 40,000Experience: 380/ Fame: 165 |