---
id: 2126
category: Medium Sized Creature
---
# Discovery: coyote

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Medium Sized Creature |
| **Discovery Name** | coyote |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A type of dog that lives in North America. The word "coyote" means "singing dog" in the Nahuatl language, and is derived from its frequent howling behavior. It looks similar to a wolf, but is smaller than a wolf, with a body length of about 1 m. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 500 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | singing dog 



 Discovery Experience 500 Discovery Name Card Experience 250 |
| **最終更新** | 2016-05-12 20:22:24 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Singing dog](Quests/Adventure/quest_3322.md)** | Adventure quest (6) | Ecological Survey (4) Biology (6) Spanish | Reward: 205,000 / Advance payment: 20,000 Experience: 240 / Fame: 210 |