---
id: 2101
category: Medium Sized Creature
---
# Discovery: Indian antelope

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Medium Sized Creature |
| **Discovery Name** | Indian antelope |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | Indian antelope. It resembles a cow, but is more slender than a cow. It has large twisted horns. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 116 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | animal map of india |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Indian animal map](docs/Quests/Other/1-1000/quest_69.md)** | Biology学(5) | Ecological Survey(5)Biology(5) |  |