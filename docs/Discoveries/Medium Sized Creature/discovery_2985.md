---
id: 2985
category: Medium Sized Creature
---
# Discovery: pamir argali

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Medium Sized Creature |
| **Discovery Name** | pamir argali |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | Biology, which is considered a subspecies of Argali. It lives in mountains and plateaus. There is an anecdote that Marco Polo discovered it, and it has another name: Marco Polo sheep. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 370 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | 2017.02.07 New addition |
| **最終更新** | 2017-02-07 17:07:25 Pochi |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Discovery of ancestors](docs/Quests/Adventure/5001-6000/quest_5692.md)** | Adventure quest (4) | Ecological research (4) Biology (6) Portuguese (1) | Reward: 122,000 / Advance payment: 10,000 Experience: 160 / Fame: 90 |