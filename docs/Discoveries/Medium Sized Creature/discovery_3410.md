---
id: 3410
category: Medium Sized Creature
---
# Discovery: hairy bush viper

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Medium Sized Creature |
| **Discovery Name** | hairy bush viper |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A snake that lives in central Africa. Their entire body is covered with erect scales, which makes them easy to climb onto plants. It kills its prey using two types of poison: a neurotoxin that stops the opponent's movement, and a hemorrhagic poison that causes massive bleeding. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 310 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2019-07-27 17:10:14 digiharuka |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Cool snake investigation](docs/Quests/Adventure/6001-7000/quest_6326.md)** | Adventure quest (4) | Ecological Survey (3) Biology (5) Portuguese | Reward: 135,000 / Advance payment: 10,000 Experience: 170 / Fame: 95 |