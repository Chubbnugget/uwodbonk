---
id: 2113
category: Medium Sized Creature
---
# Discovery: platypus

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Medium Sized Creature |
| **Discovery Name** | platypus |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | Biology has a body like an otter and a beak of a duck. They lay eggs and raise the hatched offspring with breast milk. Males can inject poison into their enemies with the claws on their hind legs. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 14 (Rank of skills required for discovery) |
| **Experience** | 1209 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | 驚天動地 |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[驚天動地](Quests/Adventure/quest_1751.md)** | Adventure quest (9) | Ecological Survey (12) Biology (14) Malayu Tagalog (1) | Reward: 369,000 / Advance payment: 30,000 Experience: 780 / Fame: 285 |