---
id: 3607
category: Historic Site
---
# Discovery: Qait Bay Fortress

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historic Site |
| **Discovery Name** | Qait Bay Fortress |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A fortress on the peninsula of Alexandria. It was built by Mamluk Sultan Qait Bey on the site where the Great Lighthouse of Alexandria, one of the Seven Wonders, collapsed in an earthquake in the 14th century. It is believed that materials from the collapsed lighthouse were also reused for construction. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 280 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2021-08-04 19:58:04 Gaocha@Guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Large lighthouse that no longer exists](docs/Quests/Adventure/6001-7000/quest_6540.md)** | Adventure quest (2) | Archeology (3) Visualization (1) German | Reward: 123,000 / Advance payment: 10,000 Experience: 180 / Fame: 200 |