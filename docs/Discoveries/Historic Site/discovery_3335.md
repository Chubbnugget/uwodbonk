---
id: 3335
category: Historic Site
---
# Discovery: Mutitjulu Fountain

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historic Site |
| **Discovery Name** | Mutitjulu Fountain |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | This pond at the base of Uluru is said to never run dry due to the rainwater flowing down from the rocky mountain. There is a legend among the locals that the ancestor of the python named Kuniya transformed into a rainbow-colored snake and still lives in ponds. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1110 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2018-08-31 22:02:03 digiharuka |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Immortal Snake](Quests/Adventure/quest_6191.md)** | Adventure Quest (8) Mythology Era | Archeology (12) Visualization (10) Oceanian languages | Reward: 183,000 / Advance payment: 10,000 Experience: 200 |