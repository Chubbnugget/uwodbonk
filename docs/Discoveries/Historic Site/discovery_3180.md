---
id: 3180
category: Historic Site
---
# Discovery: Orloj in Prague

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historic Site |
| **Discovery Name** | Orloj in Prague |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | An astronomical clock built in Prague, the ancient capital of the Holy Roman Empire. It has both the functions of an astrolabe, an astronomical measuring instrument used by astronomers at the time, and a clock, allowing it to tell not only the time but also the positions of the sun and moon. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2017-08-29 18:04:47 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[To satisfy the desire for knowledge](docs/Quests/Adventure/5001-6000/quest_5929.md)** | Adventure quest (4) | Archeology (4) Visualization (2) Dutch (1) | Reward: 111,000 / Advance payment: 10,000 Experience: 160 / Fame: 85 |