---
id: 1017
category: Historic Site
---
# Discovery: Joggins Fossils Cliffs

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historic Site |
| **Discovery Name** | Joggins Fossils Cliffs |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A cliff where many mysterious stones can be seen. Originally, it was just a stone cliff, but due to tidal erosion, many petrified animal bones have been found here. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | 800 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | ancient cliff |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Ancient Cliff](docs/Quests/Adventure/2001-3000/quest_2998.md)** | Adventure Quest (4) B.C. | Archeology (10) Visualization (8) English | Reward: 213,000 / Advance payment: 20,000 Experience: 250 / Fame: 110 |