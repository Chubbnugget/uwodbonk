---
id: 1022
category: Historic Site
---
# Discovery: Choga Zambir

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historic Site |
| **Discovery Name** | Choga Zambir |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | The remains of a religious center built by the ancient Elamites. It is divided into three levels by connected walls, with a ziggurat built on the innermost side. It is characterized by being in a very good state of preservation. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 11 (Rank of skills required for discovery) |
| **Experience** | 1060 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “The origin of Old Testament anecdotes”

 Discovery Name Card:530 |
| **最終更新** | 2022-12-07 19:34:27 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Old Testament anecdote source](docs/Quests/Adventure/3001-4000/quest_3384.md)** | Adventure quest (5) | Archeology (11) Visualization (9) Arabic | Reward: 720,000 / Advance payment: 70,000 Experience: 670 / Fame: 270 |