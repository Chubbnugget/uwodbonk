---
id: 1045
category: Historic Site
---
# Discovery: ancient arena

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historic Site |
| **Discovery Name** | ancient arena |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A stadium where the ancient ball game "Tlachitli" is believed to have been played.

 Stadiums can be seen at other Mayan ruins, but the one in Copan is one of the largest, which shows the extent of the power they wielded at the time. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 9 (Rank of skills required for discovery) |
| **Experience** | 720 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest "Slope in the square"

 Discovery Name Card:360 |
| **最終更新** | 2022-12-09 00:24:42 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Slope in the plaza](docs/Quests/Adventure/2001-3000/quest_2300.md)** | Adventure quest (7) | Archeology (9) Visualization (7) Mayan languages | Reward: 361,000 / Advance payment: 30,000 Experience: 370 / Fame: 155 |