---
id: 2906
category: Historic Site
---
# Discovery: buffalo jump

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historic Site |
| **Discovery Name** | buffalo jump |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | The official name is "Head Smashed in Buffalo Jump." A cliff in northwestern North America. It has been used since ancient times as a place to drive and hunt the American bison (buffalo) that inhabit the surrounding grasslands, and old arrowheads have also been found. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 4 (Rank of skills required for discovery) |
| **Experience** | 490 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2016-10-29 03:49:40 Ludovico |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Hunting using terrain](Quests/Adventure/quest_5586.md)** | Adventure quest (4) | Archeology (6) Visualization (4) North American languages | Reward: 73,300 / Advance payment: 0Experience: 130 / Fame: 70 |