---
id: 3392
category: Historic Site
---
# Discovery: Ruins of ancient Shu

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historic Site |
| **Discovery Name** | Ruins of ancient Shu |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | Ruins of the ancient Shu culture that flourished before BC. It is located at the upper reaches of the Yangtze River. The ruins are extremely vast, and it is believed that other valuable items besides the vertical eye mask are hidden there. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 14 (Rank of skills required for discovery) |
| **Experience** | 1230 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2019-05-19 18:53:49 digiharuka |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Gushu Historic Site](Quests/Adventure/quest_6288.md)** | Adventure Quest (10) B.C. | Archeology (14) Visualization (12) Chinese | Reward: 409,000/Advance payment: 40,000Experience: 340 |