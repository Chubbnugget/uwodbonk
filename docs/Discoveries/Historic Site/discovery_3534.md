---
id: 3534
category: Historic Site
---
# Discovery: Temple stairs with snake head

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historic Site |
| **Discovery Name** | Temple stairs with snake head |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | At the bottom of the stairs in the temple of Chichen Itza, there is a carving of the head of Kukulcan, the Mayan god. On the vernal and autumnal equinoxes, a mysterious phenomenon can be seen where the shadow of the pyramid is reflected on the stairs, resembling the undulating body of Kukulcan. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 4 (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | pursuit quest |
| **最終更新** | 2020-12-30 11:32:20 Koumeitan |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of Mayan ruins](Quests/Other/quest_6456.md)** | Pursuit quest (4) | Archeology (14) Visualization (12) Mayan languages | Reward: 325,000 / Advance payment: 0Experience: 580 |