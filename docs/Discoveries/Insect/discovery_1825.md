---
id: 1825
category: Insect
---
# Discovery: Apollo butterfly

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Insect |
| **Discovery Name** | Apollo butterfly |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | There are red, round markings on the hind wings. I don't know if it's because it resembles the sun, but it's active only when the sun is shining. It lives in dry grasslands in the mountains. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 473 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | butterfly of the sun |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Sun Butterfly](docs/Quests/Adventure/1-1000/quest_785.md)** | Adventure Quest (5) 16th Century 3rd Period | Ecological Survey (5) Biology (7) Spanish | Reward: 48,000 / Advance payment: 0Experience: 150 / Fame: 70 |