---
id: 1661
category: Religous Architecture
---
# Discovery: konark wheel

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religous Architecture |
| **Discovery Name** | konark wheel |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A wheel on the platform of the Sun Temple in Konark. The entire temple is shaped like a horse-drawn carriage. It is said that Surya, the sun god, can soar through the heavens riding this chariot. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 3 (Rank of skills required for discovery) |
| **Experience** | 150 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest "Sun God's Carriage"

 Discovery Name Card:75 |
| **最終更新** | 2022-12-31 16:59:48 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Sun God's Carriage](docs/Quests/Adventure/1-1000/quest_788.md)** | Adventure quest (5) | Religious studies (3) Visual recognition (1) Indian languages | Reward: 78,000 / Advance payment: 0Experience: 150 / Fame: 75 |