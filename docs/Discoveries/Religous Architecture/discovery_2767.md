---
id: 2767
category: Religous Architecture
---
# Discovery: Temple of Dioscuri

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religous Architecture |
| **Discovery Name** | Temple of Dioscuri |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | This temple was built according to the legend that when the last king of Rome, Tarquinius Superbus, fought against the Roman Republic, the heroes of Greek mythology, Castor and Pollux, assisted the republican side. Only three pillars remain. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 300 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2019-09-04 23:06:07 Ludovico |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Beginning of the Roman Republic](Quests/Adventure/quest_5370.md)** | Adventure Quest (5) B.C. | Religious Studies (6) Visual Recognition (4) Spanish (1) | Reward: 122,000 / Advance payment: 10,000 Experience: 240 / Fame: 135 |