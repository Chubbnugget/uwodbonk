---
id: 1658
category: Religous Architecture
---
# Discovery: karnak luxor temple

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religous Architecture |
| **Discovery Name** | karnak luxor temple |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A group of temples that were expanded and renovated one after another by successive pharaohs. There are multiple temples, and the sight of numerous statues of gods, pharaohs, and large stone pillars is spectacular. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 4 (Rank of skills required for discovery) |
| **Experience** | 290 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | city ​​of the living |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[City of the Living](docs/Quests/Adventure/1-1000/quest_723.md)** | Adventure Quest (5) B.C. | Religious Studies (4) Visual Recognition (2) Arabic | Reward: 148,000 / Advance payment: 10,000 Experience: 170 / Fame: 90 |