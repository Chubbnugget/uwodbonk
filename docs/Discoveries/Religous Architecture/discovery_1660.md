---
id: 1660
category: Religous Architecture
---
# Discovery: pyramid of giza

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religous Architecture |
| **Discovery Name** | pyramid of giza |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | Three huge pyramids in a row. In order of size, these belonged to Khufu, Khafre, and Menkaure. It is built by precisely stacking huge stones. Can we create the same thing with current technology? |
| **Era** |  |
| **Deck Points** | 2 (Required Deck Points for debate deck) |
| **Difficulty** | 2 (Rank of skills required for discovery) |
| **Experience** | 145 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Survey of the Pyramids of Giza |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Investigation of the Pyramids of Giza](docs/Quests/Adventure/1-1000/quest_115.md)** | Adventure Quest (4) B.C. | Religious Studies (2) Visual Recognition (1) Arabic | Reward: 45,000 / Advance payment: 0Experience: 130 / Fame: 65 |