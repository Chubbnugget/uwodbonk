---
id: 3608
category: Religous Architecture
---
# Discovery: Avebury Stone Circle

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religous Architecture |
| **Discovery Name** | Avebury Stone Circle |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | Prehistoric Era ruins on the British Isles. The stone circle lined with giant stones within the ruins is so vast that a small village could fit inside it. The purpose of the ruins is unknown, but perhaps rituals were also performed there. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2021-08-04 23:32:22 Hiyokomaru |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of giant stone ruins](docs/Quests/Other/6001-7000/quest_6541.md)** | Pursuit quest (3) | Religious Studies (10) Visual Recognition (8) English | Reward: 160,000 / Advance payment: 0Experience: 500 |