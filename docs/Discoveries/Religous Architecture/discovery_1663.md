---
id: 1663
category: Religous Architecture
---
# Discovery: Sainte Chapelle

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religous Architecture |
| **Discovery Name** | Sainte Chapelle |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A Christian chapel built on the Ile de la Cité in Paris. It contains many holy relics. It is a building that expresses the splendor of Gothic architecture, with magnificent stained glass windows covering the windows. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 340 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | A jewel built in Paris |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Jewel built in Paris](docs/Quests/Adventure/3001-4000/quest_3154.md)** | Adventure quest (9) | Religious Studies (5) Visual Recognition (3) French (1) | Reward: 72,000 / Advance payment: 0Experience: 130 / Fame: 65 |