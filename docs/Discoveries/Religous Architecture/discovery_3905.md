---
id: 3905
category: Religous Architecture
---
# Discovery: Big Wild Goose Pagoda

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religous Architecture |
| **Discovery Name** | Big Wild Goose Pagoda |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A square brick pagoda built inside a temple in Xi'an. It is said that it was built by Xuanzang at the request of the emperor to store the scriptures and Buddhist statues he brought back from India. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 15 (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2024-10-17 13:06:12 Nicolette |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Tower built by Tang monk Xuanzang](docs/Quests/Adventure/6001-7000/quest_6893.md)** | Adventure quest (10) | Religious Studies (15) Visual Recognition (13) Chinese | Remuneration: 301,000 / Advance payment: 30,000 |