---
id: 1667
category: Religous Architecture
---
# Discovery: Nantowas

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religous Architecture |
| **Discovery Name** | Nantowas |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | The most magnificent structure in the Nanmadol ruins. It is called ``Inside the Chief's Mouth''. It is a sacred place, and the approach to the shrine is divided into one for nobles and one for ordinary people. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | 930 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | island ritual |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Island Ritual](docs/Quests/Adventure/2001-3000/quest_2270.md)** | Adventure quest (9) | Religious studies (10) Visual recognition (8) Oceanian languages | Reward: 144,000 / Advance payment: 10,000 Experience: 190 / Fame: 90 |