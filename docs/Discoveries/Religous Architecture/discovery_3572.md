---
id: 3572
category: Religous Architecture
---
# Discovery: Baalbek megalith

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religous Architecture |
| **Discovery Name** | Baalbek megalith |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A huge stone in the ruins of Paarlbeck. The ruins consist of three temples dedicated to three of the Roman gods, one of which, the Temple of Jupiter, is based on three megaliths known as the Paarbek megaliths. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2021-06-30 21:04:48 Hiyokomaru |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of Baalbek](docs/Quests/Other/6001-7000/quest_6498.md)** | Pursuit quest (5) | Religious Studies (17) Visualization (15) Greek (1) | Reward: 500,000 / Advance payment: 0Experience: 1,488 |