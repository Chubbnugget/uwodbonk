---
id: 3541
category: Religous Architecture
---
# Discovery: Monte Alban Observatory

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religous Architecture |
| **Discovery Name** | Monte Alban Observatory |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | Monte Albán is said to have its origins in a defensive village built on top of a hill to control key areas. The ruins have unique buildings and an observatory, and are inlaid with stone tablets inscribed with the names of conquered places. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2020-11-25 06:01:05 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of the mountaintop ruins](docs/Quests/Other/6001-7000/quest_6463.md)** | Pursuit quest (4) | Religious Studies (14) Visualization (12) Nahuatl (1) | Remuneration: 250,000/Advance payment: 0 |