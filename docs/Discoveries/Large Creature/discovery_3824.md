---
id: 3824
category: Large Creature
---
# Discovery: shetland pony

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Large Creature |
| **Discovery Name** | shetland pony |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A horse native to the Shetland Islands in the northeast of England. Although the body is small, it is strong and strong. There are various coat colors such as gray, blue, brown, black, and white. It has been used since ancient times for farming and transporting goods. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 220 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | "Habitat map of subarctic animals"

 Discovery Name Card:110 |
| **最終更新** | 2023-09-20 03:23:14 Salmon can |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Subarctic animal habitat map](docs/Quests/Other/6001-7000/quest_6796.md)** | Biology学(7) | Ecological Survey(7)Biology(7) | Experience：220 |