---
id: 2244
category: Large Creature
---
# Discovery: boa

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Large Creature |
| **Discovery Name** | boa |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A large snake that lives in a wide range of environments, from grasslands to forests. When attacking birds or large lizards, they suffocate the prey with their thick bodies and then swallow it whole. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 280 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | [Imperial Order] [[Threat of advancing into the New World |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Threat of expansion into the new continent](Quests/Other/quest_1147.md)** | Imperial command quest (5) | Ecological research (3) Biology (5) |  |
| **[Things that tighten](Quests/Adventure/quest_1246.md)** | Adventure quest (5) | Ecological research (3) Biology (5) Mayan languages | Reward: 90,000 / Advance payment: 0Experience: 225 / Fame: 110 |