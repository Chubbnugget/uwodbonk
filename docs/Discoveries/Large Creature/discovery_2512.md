---
id: 2512
category: Large Creature
---
# Discovery: persian leopard

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Large Creature |
| **Discovery Name** | persian leopard |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A subspecies of the leopard family that lives in the Middle East. It is the largest subspecies of leopard. Unlike normal leopards, it also has spots on its chest and belly. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 220 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | large leopard map |
| **最終更新** | 2016-01-23 08:30:59 Ludovico |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Large Leopard Map](docs/Quests/Other/4001-5000/quest_4872.md)** | Biology学(6) | Ecological Survey(6)Biology(6) | Experience：220 |