---
id: 2052
category: Small Creature
---
# Discovery: basilisk

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | basilisk |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | It is a lizard with a total length of about 70cm. More than half of its body length is its tail, its hind limbs are well-developed, and it runs on two legs. Their toes, especially their middle fingers, are very long, allowing them to run through the water. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 9 (Rank of skills required for discovery) |
| **Experience** | 720 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | lizard running on water |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Lizard running on water](Quests/Adventure/quest_712.md)** | Adventure quest (8) | Ecological Survey (7) Biology (9) Spanish | Reward: 250,000 / Advance: 0Experience: 210 / Fame: 110 |