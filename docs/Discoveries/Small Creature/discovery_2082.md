---
id: 2082
category: Small Creature
---
# Discovery: european mink

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | european mink |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | The coat is dark brown to black, with a white stripe on the lip. They are nocturnal and feed on small creatures such as birds, mammals, and fish. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 478 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | enchanted fur |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Enchanted Fur](docs/Quests/Adventure/1001-2000/quest_1011.md)** | Adventure quest (6) | Ecological Survey (5) Biology (7) Norse | Reward: 79,000 / Advance payment: 0Experience: 160 / Fame: 75 |