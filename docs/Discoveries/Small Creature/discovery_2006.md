---
id: 2006
category: Small Creature
---
# Discovery: frilled lizard

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | frilled lizard |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A lizard that lives in Australia. The body length, including the long tail, can reach up to 1 m. It is characterized by a collar that covers its neck, and when it encounters an enemy, it unfolds and opens its mouth wide to intimidate. Then, when the opponent flinches, he runs away on two legs. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 330 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Lizard running on two legs |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Lizard running on two legs](docs/Quests/Adventure/1001-2000/quest_1571.md)** | Adventure quest (5) | Ecological research (4) Biology (6) Oceanian languages | Reward: 77,000 / Advance payment: 0Experience: 280 / Fame: 184 |