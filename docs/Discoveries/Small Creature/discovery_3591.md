---
id: 3591
category: Small Creature
---
# Discovery: oriofrinella

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | oriofrinella |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A type of frog that lives only in specific highlands and is said to have evolved uniquely to suit its environment. It is said to be characterized by the fact that it does not have a tadpole period and hatches directly from the egg in the form of a frog, and that it does not have webs and cannot swim or jump. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2021-06-29 12:30:25 Hiyokomaru |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of strange frog](docs/Quests/Other/6001-7000/quest_6518.md)** | Pursuit quest (4) | Ecological research (12) Biology (14) West African languages | Reward: 455,000 / Advance payment: 0Experience: 1,720 / Fame: 390 |