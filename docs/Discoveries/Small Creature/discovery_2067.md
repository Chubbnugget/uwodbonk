---
id: 2067
category: Small Creature
---
# Discovery: black mamba

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | black mamba |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A highly venomous snake that lives in Africa. They move so fast that they can even catch up with running humans. It can be said that Biology is extremely dangerous. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 270 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | A poisonous snake that cannot escape |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Inescapable poisonous snake](Quests/Adventure/quest_875.md)** | Adventure quest (6) | Ecological research (3) Biology (5) Swahili | Reward: 89,000 / Advance payment: 0Experience: 140 / Fame: 70 |