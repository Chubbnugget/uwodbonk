---
id: 3499
category: Small Creature
---
# Discovery: winter coat stoat

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | winter coat stoat |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | The stoat's fur has turned pure white in winter. In old legends, it is also considered a symbol of purity. For this reason, the winter stoat fur called ermine and clothing made from fur were favored by aristocrats and royalty. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 3 (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2020-07-26 21:45:39 Koumeitan |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of ermine](Quests/Other/quest_6420.md)** | Pursuit quest (3) | Ecological Survey (8) Biology (10) English | Reward: 180,000 |