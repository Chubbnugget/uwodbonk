---
id: 2013
category: Small Creature
---
# Discovery: kangaroo rat

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | kangaroo rat |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A rat that jumps on its highly developed hind legs. It is large for a rat. |
| **Era** |  |
| **Deck Points** | 1 (Required Deck Points for debate deck) |
| **Difficulty** | 3 (Rank of skills required for discovery) |
| **Experience** | 60 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | bouncing mouse map |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Bouncing Mouse Map](docs/Quests/Other/1001-2000/quest_1491.md)** | Biology学(3) | Ecological Survey(3)Biology(3) |  |