---
id: 2056
category: Small Creature
---
# Discovery: pygmy marmoset

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | pygmy marmoset |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A very small monkey. It only grows to a size that can fit in the palm of your hand. It prefers forests along rivers and lives mainly by ingesting tree sap. It makes a sound like a bird. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | 700 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | monkey on the palm |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Monkey on the palm](docs/Quests/Adventure/1001-2000/quest_1935.md)** | Adventure quest (7) | Ecological Survey (8) Biology (10) French | Reward: 218,000 / Advance payment: 20,000 Experience: 260 / Fame: 110 |