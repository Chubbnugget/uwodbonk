---
id: 2011
category: Small Creature
---
# Discovery: chameleon

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | chameleon |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A lizard about 50cm long. It can change its body color to match the color of its surroundings. It also seems to change when he gets excited. They live in trees and catch insects by extending their tongues like frogs. Two toes are opposite each other, which seems to be useful for clinging to trees. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 473 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | seven changing lizards |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Seven Changing Lizard](docs/Quests/Adventure/1-1000/quest_630.md)** | Adventure quest (7) | Ecological research (5) Biology (7) Swahili | Reward: 180,000 / Advance payment: 0Experience: 190 / Fame: 100 |