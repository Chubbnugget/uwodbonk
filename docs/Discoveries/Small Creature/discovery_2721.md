---
id: 2721
category: Small Creature
---
# Discovery: Linkarus

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | Linkarus |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A snake that lives in southern Africa. Not only is it poisonous, but when it senses danger, it has a habit of spitting venom into the enemy's eyes. They are good at pretending to be dead, so it would be dangerous to approach them without caution. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | 770 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2020-09-12 17:48:45 Pikopiko |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Poisonous snake](Quests/Adventure/quest_5309.md)** | Adventure quest (9) | Ecological research (8) Biology (10) West African languages | Reward: 309,000 / Advance payment: 30,000 Experience: 280 / Fame: 165 |