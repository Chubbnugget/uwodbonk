---
id: 2032
category: Small Creature
---
# Discovery: Semal box turtle

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | Semal box turtle |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A turtle that lives near freshwater and has a large dome-shaped shell. The gap between the dorsal carapace and plastron can be completely closed. There are many different head colors, including brown and dark green. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 200 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | yellow turtle map |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Yellow Turtle Map](docs/Quests/Other/2001-3000/quest_2530.md)** | Biology学(6) | Ecological Survey(6)Biology(6) |  |