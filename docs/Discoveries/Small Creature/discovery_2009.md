---
id: 2009
category: Small Creature
---
# Discovery: frog bat

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | frog bat |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | This bat lives in the rainforests of Central America, and as its name suggests, it preys on frogs. The ability to tell whether a frog is poisonous or not based on its cry is probably due to its ears, which are larger than its head. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 310 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | [Royal Order] [[Biology research in the field] |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Biology that resists poison](docs/Quests/Adventure/2001-3000/quest_2310.md)** | Adventure quest (4) | Ecological research (3) Biology (5) Maya languages ​​(1) | Reward: 145,000 / Advance payment: 10,000 Experience: 190 / Fame: 180 |
| **[On-site Biology research](docs/Quests/Other/1001-2000/quest_1902.md)** | Imperial command quest (4) | Ecological research (3) Biology (5) |  |