---
id: 3943
category: Small Creature
---
# Discovery: brown basilisk

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | brown basilisk |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A type of lizard with a sail-like mane called a crest. It lives in the rainforests of Central and South America. It has a brown body and is said to be the largest member of the basilisk family. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 9 (Rank of skills required for discovery) |
| **Experience** | 780 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Discovery Name Card:390 |
| **最終更新** | 2025-04-06 19:55:40 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of the lizard that causes miracles](Quests/Other/quest_6931.md)** | Pursuit quest (7) | Ecological research (7) Biology (9) Spanish (1) | Reward: 220,000Experience: 380/ Fame: 285 |