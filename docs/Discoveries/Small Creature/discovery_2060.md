---
id: 2060
category: Small Creature
---
# Discovery: possum anteater

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | possum anteater |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A marsupial that feeds on ants. It destroys ant nests with its large clawed front paws. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 160 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Map of animals that eat ants |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Map of animals that eat ants](docs/Quests/Other/1001-2000/quest_1459.md)** | Biology学(6) | Ecological Survey(6)Biology(6) | Experience：160 |