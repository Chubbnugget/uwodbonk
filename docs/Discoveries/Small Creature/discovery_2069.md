---
id: 2069
category: Small Creature
---
# Discovery: polymita

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | polymita |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A type of snail that lives in some regions of Cuba. The soft body part is dark blue, and each individual has a different colored shell. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1330 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Living dragonfly ball |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Living dragonfly ball](Quests/Adventure/quest_3803.md)** | Adventure quest (10) | Ecological Survey (10) Biology (12) Spanish | Reward: 260,000 / Advance payment: 20,000 Experience: 290 / Fame: 125 |