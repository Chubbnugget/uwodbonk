---
id: 3710
category: Small Creature
---
# Discovery: Yellow-bellied turtle

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Small Creature |
| **Discovery Name** | Yellow-bellied turtle |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A turtle that lives in the Pascagoula River in the Mississippi Basin. The carapace has spike-like protrusions and is mottled with yellow or orange. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 300 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-10-11 08:15:34 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Map of a turtle with a protrusion on its back](docs/Quests/Other/6001-7000/quest_6656.md)** | Biology学(8) | Ecological Survey(8)Biology(8) |  |