---
id: 1261
category: Historical Relics
---
# Discovery: legendary canoe

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | legendary canoe |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | This canoe is said to have been used to migrate from Hawaiki to Aotearoa. It has been preserved in a fairly old state, so it may actually be the canoe that Kupe and his friends used to migrate. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 13 (Rank of skills required for discovery) |
| **Experience** | 1196 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | legendary canoe |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Legendary Canoe](docs/Quests/Adventure/2001-3000/quest_2014.md)** | Adventure quest (9) | Exploration (11) Archeology (13) Unlocking (11) | Reward: 267,000 / Advance payment: 20,000 Experience: 420 / Fame: 140 |