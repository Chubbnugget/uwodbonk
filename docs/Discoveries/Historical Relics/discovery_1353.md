---
id: 1353
category: Historical Relics
---
# Discovery: Queen's Lila with Bull Head

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | Queen's Lila with Bull Head |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A lyre discovered in the tomb of Queen Pu Abi in the Royal Tombs of Ur. Gold is used to decorate the bull's head. Lapis lazuli and shellfish are also used in abundance, demonstrating the richness of the time. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 390 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | The glory of Ur |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Prosperity of Ur](Quests/Adventure/quest_82.md)** | Adventure Quest (6) B.C. | Exploration (4) Archeology (6) Arabic | Reward: 71,000 / Advance payment: 0Experience: 150 / Fame: 75 |