---
id: 1323
category: Historical Relics
---
# Discovery: colored bricks

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | colored bricks |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | It is a beautifully colored brick. Bricks of different colors are combined and used to draw patterns on walls and gates. |
| **Era** |  |
| **Deck Points** | 2 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 99 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | colored brick map |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Colored brick map](Quests/Other/quest_604.md)** | Archeology(5) | Exploration(5)Archaeology(5) |  |