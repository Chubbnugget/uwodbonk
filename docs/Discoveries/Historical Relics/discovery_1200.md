---
id: 1200
category: Historical Relics
---
# Discovery: Paspa stone piece

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | Paspa stone piece |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A piece of stone that is said to have been carved with characters used in the Mongol Empire. It seems to be read from top to bottom. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 13 (Rank of skills required for discovery) |
| **Experience** | 1180 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | letters that are disappearing |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Lost characters](docs/Quests/Adventure/3001-4000/quest_3729.md)** | Adventure quest (7) 14th century | Exploration (11) Archeology (13) Unlocking (11) | Remuneration: 681,000 / Advance payment: 60,000 |