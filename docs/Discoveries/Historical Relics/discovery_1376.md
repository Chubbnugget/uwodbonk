---
id: 1376
category: Historical Relics
---
# Discovery: Tiger Talisman

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | Tiger Talisman |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A tiger-shaped tally that was mainly used from the Sengoku Era to the Han Dynasty. Used during conscription.

 It is said that if the tallies did not match, the soldiers could not move. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 330 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Relics of the Three Kingdoms Era”

 Discovery Name Card:165 |
| **最終更新** | 2022-11-27 21:23:17 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Relics of the Three Kingdoms Era](docs/Quests/Adventure/2001-3000/quest_2820.md)** | Adventure quest (4) | Exploration(3)Archaeology(5)Chinese(1) | Reward: 82,000 / Advance payment: 0Experience: 140 / Fame: 70 |