---
id: 1357
category: Historical Relics
---
# Discovery: silver armor

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | silver armor |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | Viking mail left behind on the Scandinavian Peninsula. Like the helmet, it is decorated with white and silver, and belongs to the chivalrous Sorfin. It seems that it was passed down from generation to generation in Olga and Snorri's family along with an oral saga. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1100 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Poetry created by incompatible things |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Poetry created by incompatible things](docs/Quests/Adventure/3001-4000/quest_3230.md)** | Adventure quest (6) | Exploration (10) Archeology (12) Unlocking (10) | Remuneration: 130,000 / Advance payment: 20,000 |