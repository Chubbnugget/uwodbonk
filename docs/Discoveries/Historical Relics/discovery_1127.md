---
id: 1127
category: Historical Relics
---
# Discovery: clovis point

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | clovis point |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | Stone tools thought to have been used in the prehistoric Era and Clovis civilizations. The base has a unique shallow groove. It appears to have been widely used for hunting mammoths and bison. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 490 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | history of mankind |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[History of Humankind](Quests/Adventure/quest_3011.md)** | Adventure Quest (5) B.C. | Exploration (5) Archeology (7) Unlocking (5) | Reward: 231,000 / Advance payment: 20,000 Experience: 270 / Fame: 115 |