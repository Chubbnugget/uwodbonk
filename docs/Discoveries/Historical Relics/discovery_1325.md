---
id: 1325
category: Historical Relics
---
# Discovery: painted bark harness

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | painted bark harness |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A mudguard made of birch bark that is worn on horses. If you look closely, there are signs that something was drawn on it. It was probably brightly colored. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 500 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Cooperation with research on Silla relics |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Silla relics research cooperation](Quests/Adventure/quest_2734.md)** | Adventure quest (4) | 探索(5)考古学(7)朝鮮語 | Reward: 133,000 / Advance payment: 10,000 Experience: 190 / Fame: 85 |