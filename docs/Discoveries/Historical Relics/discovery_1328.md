---
id: 1328
category: Historical Relics
---
# Discovery: push type

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | push type |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A mold made of earthenware used to create patterns. It seems that many patterns were designed. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 280 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | embossed stone |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Relief Stone](Quests/Adventure/quest_1282.md)** | Adventure Quest (5) B.C. | Exploration (3) Archeology (5) Unlocking (3) | Reward: 69,000 / Advance payment: 0Experience: 270 / Fame: 95 |