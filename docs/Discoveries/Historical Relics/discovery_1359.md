---
id: 1359
category: Historical Relics
---
# Discovery: stone arrowhead

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | stone arrowhead |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | Heckler. It is a stone tool from the Era and has a shape that can be seen in any civilization. Could it be that hunting laws were the same everywhere? |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 4 (Rank of skills required for discovery) |
| **Experience** | 160 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Hunting life in Anatolia |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Hunting life in Anatolia](docs/Quests/Adventure/1-1000/quest_16.md)** | Adventure Quest (5) B.C. | Exploration (2) Archeology (4) Unlocking (2) | Reward: 63,000 / Advance payment: 0Experience: 140 / Fame: 65 |