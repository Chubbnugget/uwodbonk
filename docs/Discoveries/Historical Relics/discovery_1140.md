---
id: 1140
category: Historical Relics
---
# Discovery: Cote d'Izy fish scale pottery

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | Cote d'Izy fish scale pottery |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | "Gyorin Sai Mondoki". Painted earthenware with a fish scale pattern excavated from ruins spread across the lower reaches of the Indus River. It can also be said to be the forerunner of the Indus civilization. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 290 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | life of the ancients |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Life of the Ancients](Quests/Adventure/quest_559.md)** | Adventure Quest (6) B.C. | Exploration (3) Archeology (5) Indian languages | Reward: 104,000 / Advance payment: 0Experience: 140 / Fame: 75 |