---
id: 2842
category: Historical Relics
---
# Discovery: Mancunium Fortress Stone

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | Mancunium Fortress Stone |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A stone fragment used as building material for forts in the midwestern part of Britain. It appears to be from Mancunium, an outpost built by the ancient Roman Empire. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 363 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest "Origin of Manchester"

 Discovery Name Card:330 |
| **最終更新** | 2022-12-02 20:28:27 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Origin of Manchester](Quests/Adventure/quest_5511.md)** | Adventure quest (3) | Explore(3)Archaeology(5)English | Reward: 107,000/Advance payment: 10,000Experience: 150 |