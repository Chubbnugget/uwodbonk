---
id: 987
category: Fossils
---
# Discovery: grass pattern stone

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Fossils |
| **Discovery Name** | grass pattern stone |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A stone with a plant pattern that resembles a fern. Although it is called a fern, it is very large. Why do leaves turn to stone? Or is it something artificial? |
| **Era** |  |
| **Deck Points** | 1 (Required Deck Points for debate deck) |
| **Difficulty** | 3 (Rank of skills required for discovery) |
| **Experience** | 48 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | strange stone map

 Landing on the southern coast of Africa |
| **最終更新** | 2015-12-29 08:35:08 Gottan |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Strange Stone Map](docs/Quests/Other/1-1000/quest_415.md)** | Biology学(3) | Explore(3)Biology(3) |  |