---
id: 993
category: Fossils
---
# Discovery: long neck dragon bone

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Fossils |
| **Discovery Name** | long neck dragon bone |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A giant Biology bone with a neck as long as the body. The total length is over 20m. However, although the shape is bone, it has changed to resemble stone. Does such a biology really exist? It has no horns or wings for a dragon bone, so it may be biology that is told in other legends. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 506 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | abnormal dragon bones |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Abnormal Dragon Bone](docs/Quests/Adventure/1-1000/quest_339.md)** | Adventure Quest (7) B.C. | Exploration (5) Biology (7) Arabic | Reward: 122,000 / Advance payment: 10,000 Experience: 150 / Fame: 80 |