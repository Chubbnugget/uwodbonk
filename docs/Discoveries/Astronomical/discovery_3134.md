---
id: 3134
category: Astronomical
---
# Discovery: Giraffe

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Astronomical |
| **Discovery Name** | Giraffe |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | It is located in the constellation Stars in the northern sky, near the North Pole. Although there are some areas where it can be observed throughout the year, there are no noticeable Stars and it is not well-known. The origin of the name is not the giraffe, which is a divine beast in China, but the giraffe, which lives in Africa. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 15 (Rank of skills required for discovery) |
| **Experience** | 1410 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Legendary Spirit Beast”

 Discovery Name Card:705 |
| **最終更新** | 2022-11-20 21:24:21 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Legendary Spiritual Beast](Quests/Adventure/quest_5876.md)** | Adventure quest (10) | Astronomy (17) Visual recognition (15) Korean | Reward: 875,000 / Advance payment: 80,000 Experience: 630 / Fame: 400 |