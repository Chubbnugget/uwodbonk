---
id: 3103
category: Astronomical
---
# Discovery: southern coal sack

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Astronomical |
| **Discovery Name** | southern coal sack |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A dark Stars cloud with the name "coal bag". It can be seen near the constellation Minami Jugis. Because it has the Milky Way in its background, it can sometimes be observed with the naked eye. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 11 (Rank of skills required for discovery) |
| **Experience** | 700 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest "Black Streak Floating in the Sky"

 Discovery Name Card:350 |
| **最終更新** | 2022-11-09 20:51:21 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Black streaks floating in the sky](docs/Quests/Adventure/5001-6000/quest_5845.md)** | Adventure quest (8) | Astronomy (11) Visual recognition (9) West African languages | Reward: 332,000 / Advance payment: 30,000 Experience: 290 / Fame: 175 |