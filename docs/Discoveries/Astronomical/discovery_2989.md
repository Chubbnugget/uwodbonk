---
id: 2989
category: Astronomical
---
# Discovery: Tuban

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Astronomical |
| **Discovery Name** | Tuban |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | One of the stars that make up the constellation Draco. It used to be the North Pole Stars, but due to precession, it is no longer called the North Pole Stars. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 630 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Another North Pole Stars”

 Discovery Name Card:315 |
| **最終更新** | 2022-11-03 22:04:16 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Another North Pole Stars](Quests/Adventure/quest_5696.md)** | Adventure quest (6) | Astronomy (8) Visualization (6) Arabic | Reward: 404,000 / Advance payment: 4,000 Experience: 340 / Fame: 205 |