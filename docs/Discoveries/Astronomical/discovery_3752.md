---
id: 3752
category: Astronomical
---
# Discovery: Praequipua

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Astronomical |
| **Discovery Name** | Praequipua |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | The brightest Stars in the constellation Porthena. It is a dark star of about 4th magnitude, located at the butt of the Little Lion. The name means ``major thing'' in Latin, but this name itself is no longer used very often. |
| **Era** | 17世紀第1期 |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | 900 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Discovery Name Card: 450 |
| **最終更新** | 2022-12-21 12:59:20 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Praequipua](docs/Quests/Other/6001-7000/quest_6710.md)** | Celestial Object Discovery Name(10)17th Century Issue 1 | Astronomy (10) Visualization (10) | Experience：900 |