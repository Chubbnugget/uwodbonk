---
id: 3124
category: Astronomical
---
# Discovery: Elongated galaxy in Ursa Major

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Astronomical |
| **Discovery Name** | Elongated galaxy in Ursa Major |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | An elongated galaxy located in the constellation Ursa Major. It is difficult to observe with the naked eye, so it requires specialized equipment. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 15 (Rank of skills required for discovery) |
| **Experience** | 1370 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Discovery Name Card:685 |
| **最終更新** | 2022-10-12 20:29:12 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Smoker Stars](Quests/Adventure/quest_5864.md)** | Adventure quest (10) | Astronomy (17) Visualization (15) French | Reward: 236,000/ Advance: 20,000Experience: 230/ Fame: 135 |