---
id: 2596
category: Astronomical
---
# Discovery: Mizar and Alcor

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Astronomical |
| **Discovery Name** | Mizar and Alcor |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | Mizar is the second star from the tip of the dipper handle of the Hokuto Stars. Alcor is a 4th class Stars right next to Mizar. Since ancient times, the ability to distinguish between these two Stars has been used to test visual acuity. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 250 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Soldier Conditions”

 Discovery Name Card:125 |
| **最終更新** | 2022-11-03 17:44:56 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Soldier conditions](Quests/Adventure/quest_4957.md)** | Adventure quest (2) | Astronomy (4) Visualization (2) Spanish (1) | Reward: 90,000 / Advance payment: 0Experience: 150 / Fame: 70 |