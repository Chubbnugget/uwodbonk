---
id: 2856
category: Astronomical
---
# Discovery: Chokokugugus constellation

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Astronomical |
| **Discovery Name** | Chokokugugus constellation |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | Stars constellation in the southern sky established by Lacaille. The subject matter is the tools used in carving. Since there are no bright stars and the arrangement is not distinctive, you can find it by looking southeast from the constellation Orion, which is centered in the south. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 11 (Rank of skills required for discovery) |
| **Experience** | 830 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Art-themed Stars” 

 Discovery Name Card:415 |
| **最終更新** | 2022-11-30 19:55:29 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Art-themed Stars](Quests/Adventure/quest_5517.md)** | Adventure quest (8) 18th century third period | Astronomy (11) Visualization (9) French (1) | Reward: 216,000/ Advance: 20,000Experience: 220/ Fame: 125 |