---
id: 3881
category: Astronomical
---
# Discovery: Zuben El Genubi

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Astronomical |
| **Discovery Name** | Zuben El Genubi |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | Alpha Stars in Libra. 3rd class Stars, multiple Stars consisting of 4 or more permanent Stars. Its name means "southern claw" in Arabic, and is said to be a remnant of the part that originally corresponded to the claw of the constellation Scorpius. |
| **Era** | 15世紀第2期 |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 510 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Discovery Name Card:255 |
| **最終更新** | 2024-07-05 22:27:59 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Zuben El Geneubi](Quests/Other/quest_6868.md)** | Celestial Object Discovery Name(7)15th Century Issue 2 | Astronomy(7)Visualization(7) |  |