---
id: 3127
category: Astronomical
---
# Discovery: Taurus Stars Cloud

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Astronomical |
| **Discovery Name** | Taurus Stars Cloud |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A Stars cloud thought to have been created by the explosion of Stars. The scale of the explosion can be seen in the fact that there are records from all over the world that are believed to have witnessed the explosion several hundred years ago. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 15 (Rank of skills required for discovery) |
| **Experience** | 1430 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Burning of Stars”

 Discovery Name Card:715 |
| **最終更新** | 2022-11-30 20:41:42 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Stars on fire](docs/Quests/Adventure/5001-6000/quest_5867.md)** | Adventure quest (10) | Astronomy (17) Visualization (15) German | Reward: 1,187,000/Advance payment: 110,000Experience: 930 |