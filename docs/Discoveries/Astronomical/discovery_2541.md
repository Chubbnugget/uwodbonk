---
id: 2541
category: Astronomical
---
# Discovery: Double Stars in Perseus

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Astronomical |
| **Discovery Name** | Double Stars in Perseus |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | The Stars constellation in the Perseus constellation. You can see two clusters of countless Stars lined up. Even with the naked eye, you can see the dim light. |
| **Era** | 17世紀第1期 |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 9 (Rank of skills required for discovery) |
| **Experience** | 740 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Double Stars in Perseus |
| **最終更新** | 2021-07-28 02:18:14 Farley |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Double Stars of Perseus](docs/Quests/Other/4001-5000/quest_4861.md)** | Celestial Object Discovery Name(9)17th Century Issue 1 | Astronomy(9) |  |