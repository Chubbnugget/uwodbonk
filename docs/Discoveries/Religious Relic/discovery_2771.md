---
id: 2771
category: Religious Relic
---
# Discovery: Biblioteke

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religious Relic |
| **Discovery Name** | Biblioteke |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A book that systematically summarizes Greek mythology from the origin of the world to the Trojan War. It describes old Greek mythology before it was influenced by Roman mythology. |
| **Era** |  |
| **Deck Points** | 2 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 190 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Discovery Name Card:95 |
| **最終更新** | 2022-10-13 07:40:18 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Map of ancient books](docs/Quests/Other/5001-6000/quest_5385.md)** | Religious Studies(5) | Religious Studies(5)Exploration(5) | Experience：190 |