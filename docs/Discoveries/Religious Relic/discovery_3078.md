---
id: 3078
category: Religious Relic
---
# Discovery: prayer wheel

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religious Relic |
| **Discovery Name** | prayer wheel |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A cylindrical Buddhist utensil. A mantra is engraved on the outside, and a sutra is stored inside. It is often used in Tibetan Buddhism, and it is said that spinning it once will give you the same amount of merit as reading the sutra inside once. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 390 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2017-04-16 21:19:14 digiharuka |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[How to chant a mantra](docs/Quests/Adventure/5001-6000/quest_5810.md)** | Adventure quest (2) | Religious Studies (6) Exploration (4) Malayu Tagalog | Reward: 230,000/ Advance: 20,000Experience: 230/ Fame: 135 |