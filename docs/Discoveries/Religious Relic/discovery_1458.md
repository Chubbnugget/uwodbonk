---
id: 1458
category: Religious Relic
---
# Discovery: catacomb stone tablets

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Religious Relic |
| **Discovery Name** | catacomb stone tablets |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A mysterious stone tablet found deep within the catacombs. There appears to be some writing carved into it, but the surface is so damaged that it seems impossible to decipher it. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 330 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | dungeon |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Syracuse Church Basement (Middle Level)](Quests/Other/quest_2773.md)** | Dungeon (1) | - |  |