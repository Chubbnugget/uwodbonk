---
id: 802
category: Treasure
---
# Discovery: Five-stringed biwa

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | Five-stringed biwa |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | The pronunciation is "Gogen no Biwa". A simple biwa made in Japan. It seems that a beautiful five-stringed pipa is kept in a treasure trove that was said to have been a gift from India to Japan long ago, but the details are unknown. It seems that there are only a limited number of people who can see the real thing. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 690 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | traditional stringed instruments |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Traditional stringed instruments](docs/Quests/Adventure/2001-3000/quest_2500.md)** | Adventure quest (6) | 探索(6)Treasures鑑定(8)日本語(1) | Reward: 71,000 / Advance payment: 0Experience: 130 / Fame: 65 |