---
id: 835
category: Treasure
---
# Discovery: fairy whisper

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | fairy whisper |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A huge opal that weighs well over 2000 carats. It is transparent and you can see it sparkling beautifully inside. Legend has it that a fairy lives in this stone and will grant the owner's wishes if he likes it. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 13 (Rank of skills required for discovery) |
| **Experience** | 1170 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | A jewel where fairies live

 Discovery Experience1170 Discovery Name Card Experience585 |
| **最終更新** | 2016-07-30 23:16:42 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Jewel where fairies live](docs/Quests/Adventure/1001-2000/quest_1728.md)** | Adventure quest (9) | Exploration (11) Treasures Appraisal (13) Unlocking (11) | Reward: 305,000 / Advance payment: 30,000 Experience: 490 / Fame: 200 |