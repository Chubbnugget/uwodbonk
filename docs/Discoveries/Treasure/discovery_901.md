---
id: 901
category: Treasure
---
# Discovery: faded pirate flag

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | faded pirate flag |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | It looks like the flag of a pirate ship. Pirate flags are used by pirates to identify their crew members and are a source of pride. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1190 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | pirate symbol |
| **最終更新** | 2020-11-23 13:18:38 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Pirate Symbol](Quests/Adventure/quest_3725.md)** | Adventure Quest (9) 17th Century 4th Period | Exploration (10) Treasures Appraisal (12) Unlocking (10) | Reward: 1,322,000 / Advance payment: 30,000 Experience: 340 / Fame: 290 |