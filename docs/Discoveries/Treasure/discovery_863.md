---
id: 863
category: Treasure
---
# Discovery: Weapon operation

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | Weapon operation |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A work by Count Maurits, governor of the Low Countries. Count Maurits divided military training methods into smaller sections and succeeded in increasing the level of understanding of each step. This information has been compiled in this book without concealing it. It is said that the proficiency of each army has improved significantly. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 740 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | A military book for mercenaries 



 Discovery Experience740 Discovery Name Card Experience370 |
| **最終更新** | 2016-06-07 10:36:55 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[War book for mercenaries](Quests/Adventure/quest_3311.md)** | Adventure Quest (6) 16th Century 5th Period | Exploration (6) Treasures Appraisal (8) Unlocking (6) | Reward: 167,000/ Advance: 10,000 Experience: 210/ Fame: 95 |