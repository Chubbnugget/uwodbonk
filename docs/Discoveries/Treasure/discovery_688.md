---
id: 688
category: Treasure
---
# Discovery: eternal star

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | eternal star |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A huge diamond, well over 100 carats. Many large diamonds are colored, but this one is completely colorless. It is a diamond that can be called a miracle, with almost no inclusions. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | 1105 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Immortal Stars

 Discovery Experience1105 Discovery Name Card Experience552 |
| **最終更新** | 2016-05-15 19:47:26 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Immortal Stars](docs/Quests/Adventure/1-1000/quest_948.md)** | Adventure quest (9) | Exploration (8) Treasures Appraisal (10) Unlocking (8) | Reward: 30,000 / Advance payment: 0Experience: 420 |