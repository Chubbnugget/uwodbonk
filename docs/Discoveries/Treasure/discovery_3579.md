---
id: 3579
category: Treasure
---
# Discovery: cylinder music box

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | cylinder music box |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | An automatic performance device that can play music by winding a mainspring. A pin embedded in a rotating cylinder makes sound by flicking the comb teeth. It is possible to play various songs by replacing the cylinders. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 350 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-06-05 21:44:55 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Box containing music](Quests/Adventure/quest_6507.md)** | Adventure quest (4) 19th century | Exploration (3) Treasures Appraisal (5) French | Reward: 156,000 / Advance payment: 20,000 Experience: 180 / Fame: 200 |