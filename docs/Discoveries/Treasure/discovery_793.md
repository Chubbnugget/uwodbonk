---
id: 793
category: Treasure
---
# Discovery: wayang kulit

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | wayang kulit |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A traditional shadow play that has been performed on the island of Java since ancient times. Use a leather doll. In the local language, wayang refers to shadow and kuri refers to leather. Wayang Kuli Purwo is the most common performance, featuring stories from the Mahabharata and Ramayana, but Javanese legends and folk tales are also performed. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 270 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Javanese shadow play |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Javanese shadow play](docs/Quests/Adventure/1001-2000/quest_1726.md)** | Adventure quest (6) | Exploration (3) Treasures Appraisal (5) Malayu Tagalog | Reward: 85,000 / Advance payment: 0Experience: 290 / Fame: 100 |