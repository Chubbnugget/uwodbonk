---
id: 756
category: Treasure
---
# Discovery: pink diamond

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | pink diamond |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | pink diamond. The higher the transparency and the more beautiful the color, the higher the value. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 590 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | pink gemstone vein map |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Pink gemstone vein map](Quests/Other/quest_258.md)** | Treasures鑑定(12) | 探索(12)Treasures鑑定(12) | Experience：590 |