---
id: 3580
category: Treasure
---
# Discovery: wax tube phonograph

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | wax tube phonograph |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | An automatic performance device that reproduces sound by tracing the grooves carved into a wax tube with a needle and picking up the vibrations. The vibrations are transmitted to a vibrating plate called a sound box connected to the needle, creating sound, which is amplified through the horn. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 1120 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2021-03-17 03:48:26 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of automatic performance equipment](Quests/Other/quest_6508.md)** | Pursuit quest (3) 19th century | Exploration (8) Treasures Appraisal (10) Dutch | Reward: 300,000 / Advance payment: 0Experience: 1,120 |