---
id: 820
category: Treasure
---
# Discovery: synthetic bow

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | synthetic bow |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A small bow made using multiple materials. They are mainly made of wood or bamboo, but by pasting together animal bones and metal plates, their range and destructive power are increased. The detailed manufacturing method is unknown because the bow craftsman keeps the manufacturing method a secret. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1180 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Mongolian bow technique |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Mongol bow technology](docs/Quests/Adventure/3001-4000/quest_3636.md)** | Adventure quest (8) 14th century | Exploration (10) Treasures Appraisal (12) Unlocking (10) | Reward: 1,218,000/Advance payment: 120,000Experience: 1,090 |