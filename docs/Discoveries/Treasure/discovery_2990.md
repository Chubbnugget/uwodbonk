---
id: 2990
category: Treasure
---
# Discovery: shaman's coin

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | shaman's coin |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | Coins sunk to the bottom of the water near Sherman Rock. It seems that shamans who believed that rocks had the power to cause miracles would throw coins around the rocks and perform some kind of ritual. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 700 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2017-02-08 00:29:22 starseeks@guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Reconciliation ritual](docs/Quests/Adventure/5001-6000/quest_5697.md)** | Adventure quest (9) | 探索(8)Treasures鑑定(10)中国語(1) | Reward: 569,000 / Advance payment: 50,000 Experience: 440 / Fame: 275 |