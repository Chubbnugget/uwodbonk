---
id: 939
category: Treasure
---
# Discovery: 長恨歌

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | 長恨歌 |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A long Chinese poem that tells an anecdote about Emperor Xuanzong and Yang Guifei during the Tang Dynasty. There are many differences from historical facts, such as the Era being changed to the Han dynasty instead of the Tang dynasty. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 660 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | man with a book 



 Discovery Experience660 Discovery Name Card Experience330 |
| **最終更新** | 2016-05-07 14:20:19 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Man with a Book](Quests/Adventure/quest_2862.md)** | Adventure quest (5) | 探索(5)Treasures鑑定(7)中国語 | Reward: 68,000 / Advance payment: 0Experience: 130 / Fame: 65 |