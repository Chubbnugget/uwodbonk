---
id: 3535
category: Treasure
---
# Discovery: hamsa

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | hamsa |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A talisman shaped like a hand with an eye drawn in the center to protect against the evil eye. "Hamsa" means the number 5 in Arabic. There is also a theory that the Turkish amulet ``Nazar Bonciu'' is of this subspecies. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 3 (Rank of skills required for discovery) |
| **Experience** | 1040 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | pursuit quest 

 Discovery Name Card:520 |
| **最終更新** | 2023-05-11 13:51:10 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional research on amulets to ward off evil eye](Quests/Other/quest_6457.md)** | Pursuit quest (3) | Exploration (8) Treasures Appraisal (10) Turkish | Reward: 200,000 / Advance payment: 0Experience: 360 / Fame: 600 |