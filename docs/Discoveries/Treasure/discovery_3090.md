---
id: 3090
category: Treasure
---
# Discovery: collected history

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | collected history |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A history book compiled by order of the king of Il-Khan. Although it mainly describes the development of the Mongol Empire, it also collects the history of other ethnic groups, so it is called a ``collected history.'' |
| **Era** |  |
| **Deck Points** | 2 (Required Deck Points for debate deck) |
| **Difficulty** | 4 (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-11-30 18:17:47 lhj-travel |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Map of Mongolian history book](Quests/Other/quest_5830.md)** | Treasures鑑定(4) | 探索(4)Treasures鑑定(4) |  |