---
id: 951
category: Treasure
---
# Discovery: enchanted fruit

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Treasure |
| **Discovery Name** | enchanted fruit |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | Tourmaline is a mixture of pale green and bright red. Because of its color, it has been compared to a watermelon. Tourmaline is a gemstone that comes in a wide variety of colors, and many colors can be found in a single gemstone. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1170 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | beautiful and strange stones |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Beautiful and strange stone](docs/Quests/Adventure/1001-2000/quest_1357.md)** | Adventure quest (9) | Exploration (10) Treasures Appraisal (12) Unlocking (10) | Reward: 520,000 / Advance payment: 50,000 Experience: 720 / Fame: 250 |