---
id: 3661
category: Plant
---
# Discovery: Hirugidamashi

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Hirugidamashi |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | An evergreen tree found in mangrove forests along the tropical coasts of East Africa and Southeast Asia. Because it is resistant to salt, it is said that it can grow naturally on the Red Sea coast, where the soil is dry and has a high salinity concentration. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 500 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-03-27 16:48:18 NYSC |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Tree that grows in the sea](Quests/Adventure/quest_6604.md)** | Adventure quest (6) | Ecological research (5) Biology (7) Arabic (1) | Reward: 61,000/ Advance payment: 10,000 Experience: 130/ Fame: 65 |