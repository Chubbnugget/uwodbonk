---
id: 3892
category: Plant
---
# Discovery: ghost orchid

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | ghost orchid |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A type of orchid that is distributed only around the Caribbean. It has the unusual feature of growing epiphytically on trees without having leaves. The white flowers bloom from June to August, but it is extremely difficult to find them outside of the flowering season. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 15 (Rank of skills required for discovery) |
| **Experience** | 1210 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Discovery Name Card:605 |
| **最終更新** | 2024-07-06 15:58:19 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Flower without leaves](docs/Quests/Adventure/6001-7000/quest_6879.md)** | Adventure quest (10) | Ecological research (13) Biology (15) Indian languages ​​(1) | Reward: 687,000/Advance payment: 10,000Experience: 520 |