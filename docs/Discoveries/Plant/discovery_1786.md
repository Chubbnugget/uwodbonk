---
id: 1786
category: Plant
---
# Discovery: Bikakushida

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Bikakushida |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A strange plant growing from a tree trunk in the forest. It has branched leaves that resemble deer antlers. I thought it was a diseased leaf, but it turned out to be a different plant because it was attached to the plant by its roots. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 170 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Map of strangely shaped plants |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Map of strangely shaped plants](docs/Quests/Other/1001-2000/quest_1953.md)** | Biology学(6) | Ecological Survey(6)Biology(6) | Experience：170 |