---
id: 2826
category: Plant
---
# Discovery: brazil nut tree

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | brazil nut tree |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A tall tree native to the Amazon River basin. It produces fruit containing a lot of oil and fat. The shell is large and heavy, so care must be taken when harvesting. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 760 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-05-13 23:08:40 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Aguchi's contribution](docs/Quests/Adventure/5001-6000/quest_5474.md)** | Adventure quest (8) | Ecological Survey (8) Biology (10) Portuguese | Reward: 224,000 / Advance payment: 20,000 Experience: 230 / Fame: 130 |