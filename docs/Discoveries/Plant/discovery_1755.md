---
id: 1755
category: Plant
---
# Discovery: dahlia

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | dahlia |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A yellow tubular flower is attached to the center of the many layers of petals. Large ones can reach over 20 cm. |
| **Era** |  |
| **Deck Points** | 1 (Required Deck Points for debate deck) |
| **Difficulty** | 3 (Rank of skills required for discovery) |
| **Experience** | 70 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | big red flower map |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Big Red Flower Map](Quests/Other/quest_1503.md)** | Biology学(3) | Ecological Survey(3)Biology(3) |  |