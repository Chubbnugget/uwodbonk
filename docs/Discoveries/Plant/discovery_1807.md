---
id: 1807
category: Plant
---
# Discovery: purple variegus

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | purple variegus |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A plant that grows from eastern to central-southern North America. Red-purple flowers bloom from summer to autumn. The roots have medicinal properties and are used as medicine. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 700 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Request for cooperation from the trading post owner |
| **最終更新** | 2022-05-11 09:43:51 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Request for cooperation from the trading post owner](Quests/Adventure/quest_3317.md)** | Adventure quest (8) | Ecological Survey (6) Biology (8) Spanish | Reward: 262,000 / Advance payment: 20,000 Experience: 290 / Fame: 125 |