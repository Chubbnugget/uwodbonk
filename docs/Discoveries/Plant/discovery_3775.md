---
id: 3775
category: Plant
---
# Discovery: Hanabiratake

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Hanabiratake |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A flower-like mushroom that grows naturally at the base of coniferous trees such as larch. Prefers areas with high humidity and fog. It has a clean scent and refreshing texture. Because it is a rare mushroom, it is also called the ``phantom mushroom.'' |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 4 (Rank of skills required for discovery) |
| **Experience** | 210 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest "Under the dead leaves"

 Discovery Name Card:105 |
| **最終更新** | 2023-01-31 11:47:45 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Under the dead leaves](docs/Quests/Adventure/6001-7000/quest_6715.md)** | Adventure quest (4) | Ecological research (3) Biology (5) Japanese (1) | Reward: 363,500/Advance payment: 40,000Experience: 360 |