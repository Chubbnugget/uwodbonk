---
id: 3658
category: Plant
---
# Discovery: Prosopis quinella

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Prosopis quinella |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A plant of the pea family that grows naturally in the arid regions of the Middle East. Also called "Gaff tree". It has very high vitality and can grow even in drought areas such as deserts. Some trees are said to be hundreds of years old. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 500 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-05-16 13:11:08 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Tree of Life](Quests/Adventure/quest_6601.md)** | Adventure quest (6) | Ecological Survey (5) Biology (7) Arabic | Reward: 270,000/ Advance: 30,000Experience: 260/ Fame: 150 |