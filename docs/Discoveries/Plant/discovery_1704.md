---
id: 1704
category: Plant
---
# Discovery: Giant Onibus

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Giant Onibus |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A lotus with giant leaves that float on water and can reach up to 2 meters in diameter. Anyone who weighs as much as a child can ride it. The flowers change from white to red as the days pass. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 270 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | plant ship |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Plant Ship](Quests/Adventure/quest_1164.md)** | Adventure quest (4) | Ecological Survey (3) Biology (5) Spanish | Reward: 106,000 / Advance payment: 0Experience: 210 / Fame: 75 |