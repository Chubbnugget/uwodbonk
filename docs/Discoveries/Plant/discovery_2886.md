---
id: 2886
category: Plant
---
# Discovery: Gyoryubai

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Gyoryubai |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A tree native to southeastern Australia and the New Zealand archipelago. In the Maori language, it is called ``manuka.'' The nectar and leaves have various medicinal properties. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1040 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Shrubs with many medicinal properties 



 Discovery Experience1040 Discovery Name Card Experience520 |
| **最終更新** | 2016-06-19 13:23:40 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Shrubs with many medicinal properties](Quests/Adventure/quest_5557.md)** | Adventure quest (6) | Ecological research (10) Biology (12) Oceanian languages | Reward: 275,000 / Advance payment: 20,000 Experience: 260 / Fame: 150 |