---
id: 3808
category: Plant
---
# Discovery: Shaviana Trafles

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Shaviana Trafles |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A subspecies of Echeveria that is native to the desert regions of Mexico. Prefers well-ventilated, dry land. The blue-gray leaves overlap on the folds, and only the green of the leaves is pink. The colors become more vivid when they get enough sunlight. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | 860 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | pursuit quest

 Discovery Name Card:430 |
| **最終更新** | 2023-10-02 10:27:01 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of Echeveria](Quests/Other/quest_6771.md)** | Pursuit quest (7) | Ecological research (8) Biology (10) Spanish (1) | Reward: 220,000 / Advance payment: 0Experience: 380 |