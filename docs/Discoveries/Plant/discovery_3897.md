---
id: 3897
category: Plant
---
# Discovery: Epipogium affilum

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Epipogium affilum |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A type of orchid discovered on the British Isles. It has no leaves and produces small flowers. It is sometimes called the "ghost run" because it is extremely difficult to find. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | pursuit quest |
| **最終更新** | 2024-09-15 12:07:40 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of leafless flowers](docs/Quests/Other/6001-7000/quest_6884.md)** | Pursuit quest (10) | Ecological research (13) Biology (15) Mayan languages ​​(1) | Remuneration: 600,000/Advance payment: 0 |