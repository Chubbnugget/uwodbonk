---
id: 3593
category: Plant
---
# Discovery: rainbow eucalyptus

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | rainbow eucalyptus |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | As its name suggests, this tall tree is characterized by its colorful bark. As the tree ages, the outer bark peels off and new bark appears on the inside. It is said that the color of the bark changes as this process is repeated. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | 1190 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Dyeed Tree” 

 Discovery Name Card:595 |
| **最終更新** | 2023-01-31 13:21:00 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Dyeed Tree](Quests/Adventure/quest_6520.md)** | Adventure quest (10) | Ecological research (13) Biology (15) Oceanian languages | Reward: 687,000/Advance payment: 70,000Experience: 520 |