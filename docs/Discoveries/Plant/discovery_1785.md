---
id: 1785
category: Plant
---
# Discovery: Hiroha Hashidoi

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Hiroha Hashidoi |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A highly prized plant at Deoksugung Palace in the Joseon Dynasty.

 Light pink flowers bloom in clusters. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 370 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | dynasty flower

 Discovery Experience 370 Discovery Name Card Experience 185 |
| **最終更新** | 2016-07-23 17:19:44 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Dynasty Flower](Quests/Adventure/quest_2693.md)** | Adventure quest (5) | Ecological Survey (3) Biology (5) Korean | Reward: 363,000 / Advance payment: 30,000 Experience: 390 / Fame: 280 |