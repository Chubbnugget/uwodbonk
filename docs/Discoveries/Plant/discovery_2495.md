---
id: 2495
category: Plant
---
# Discovery: breadfruit

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | breadfruit |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | A plant cultivated in Oceania. The fruit can be eaten steamed or roasted. It is a useful crop that can be processed into a long-lasting emergency food. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 240 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Nuts that can be used as staple food” 

 Discovery Name Card: 120 |
| **最終更新** | 2022-11-27 17:04:59 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Nuts that can be used as staple food](Quests/Adventure/quest_4792.md)** | Adventure quest (3) 19th century | Ecological research (2) Biology (4) Oceanian languages | Reward: 94,000 / Advance payment: 0Experience: 150 / Fame: 70 |