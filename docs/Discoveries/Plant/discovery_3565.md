---
id: 3565
category: Plant
---
# Discovery: Chimera Saintpaulia

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Chimera Saintpaulia |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A mutant of the small plant Saintpaulia found only in the interior of southern Africa. Like the normal individual, the petals have a unique luster, and the mutant's unique striped pattern is also very beautiful. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 3 (Rank of skills required for discovery) |
| **Experience** | 1200 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | pursuit quest |
| **最終更新** | 2021-01-01 02:36:21 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional research on flowers blooming in the jungle](docs/Quests/Other/6001-7000/quest_6491.md)** | Pursuit quest (3) | Ecological research (8) Biology (10) West African languages | Reward: 200,000 / Advance payment: 0Experience: 1,200 |