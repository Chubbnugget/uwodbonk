---
id: 1797
category: Plant
---
# Discovery: Houou Boku

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Houou Boku |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A deciduous tree that is often found on the island of Madagascar. During the blooming period, the tree is covered with bright red flowers, giving the appearance of a blazing flame. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 280 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | great tree of flame |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Great Tree of Flame](docs/Quests/Adventure/1-1000/quest_357.md)** | Adventure quest (6) | Ecological research (3) Biology (5) Swahili | Reward: 94,000 / Advance payment: 10,000 Experience: 140 / Fame: 75 |