---
id: 3741
category: Plant
---
# Discovery: Alocasia

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Alocasia |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A plant of the Araceae family with large leaves. As its name suggests, it is not edible and is poisonous, causing severe abdominal pain. In China, it was used as a medicine based on the concept of controlling poison with poison. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 210 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Landed on the southeast coast of Taiwan Island. 

 Near the red flowers of the South Island. |
| **最終更新** | 2022-10-21 18:26:11 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Map of highly poisonous plants](Quests/Other/quest_6693.md)** | Biology学(5) | Ecological Survey(5)Biology(5) |  |