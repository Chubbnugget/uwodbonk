---
id: 1750
category: Plant
---
# Discovery: Saintpaulia

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Saintpaulia |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A small plant found only in the interior of southern Africa. It has roots in crevices in cliff rocks and has thick fleshy leaves and flowers. The petals have a unique luster and are beautiful. Since there are many mutant strains, it would be suitable for breeding. |
| **Era** |  |
| **Deck Points** | 2 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 250 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Flowers blooming quietly in the jungle |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Flowers that bloom quietly in the jungle](docs/Quests/Adventure/1001-2000/quest_1012.md)** | Adventure quest (6) | Ecological research (3) Biology (5) West African languages | Reward: 187,600 / Advance payment: 10,000 Experience: 266 / Fame: 142 |