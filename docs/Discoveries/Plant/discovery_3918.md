---
id: 3918
category: Plant
---
# Discovery: Hougane tree

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Hougane tree |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A tree native to South America. Many pale pink flowers bloom on snake-like branches. The fruit grows to about 20 cm in diameter and looks like a cannonball, hence the name. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 14 (Rank of skills required for discovery) |
| **Experience** | 530 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Discovery Name Card:265 |
| **最終更新** | 2024-11-01 22:49:47 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Map of the fruit tree](docs/Quests/Other/6001-7000/quest_6902.md)** | Biology学(14) | Ecological Survey(14)Biology(14) |  |