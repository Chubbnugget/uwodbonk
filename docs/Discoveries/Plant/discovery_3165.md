---
id: 3165
category: Plant
---
# Discovery: Death Camas

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Death Camas |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A poisonous plant distributed in the western part of the New World. As the name suggests, it is highly toxic, and if even a small amount is ingested, it can lead to organ failure and death. It is very similar to an edible plant called barracuda, so you need to be especially careful. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 15 (Rank of skills required for discovery) |
| **Experience** | 1400 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-10-22 07:37:16 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Eating poison until the end](docs/Quests/Adventure/5001-6000/quest_5901.md)** | Adventure quest (10) | Ecological Survey (15) Biology (17) Spanish | Reward: 835,000/Advance payment: 80,000Experience: 610 |