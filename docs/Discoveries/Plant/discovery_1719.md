---
id: 1719
category: Plant
---
# Discovery: Bellflower

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Plant |
| **Discovery Name** | Bellflower |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A plant that grows in the sunny mountains and fields of East Asia. The buds are shaped like balloons with connected petals, and the flowers bloom in the shape of stars. The roots are also used as herbal medicine. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 330 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | A dignified blue flower blooming |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Dignified blooming blue flower](Quests/Adventure/quest_2453.md)** | Adventure quest (4) | Ecological Survey (3) Biology (5) Japanese | Reward: 108,000 / Advance payment: 10,000 Experience: 190 / Fame: 85 |