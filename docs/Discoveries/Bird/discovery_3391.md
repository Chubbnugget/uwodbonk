---
id: 3391
category: Bird
---
# Discovery: Kereru

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Bird |
| **Discovery Name** | Kereru |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A type of pigeon that lives throughout Aotearoa, also known as the wood pigeon. Legend has it that the demigod hero Maui often transformed into this bird. It has a large body and a voracious appetite, so much so that it can swallow fruit whole, but it is said that it sometimes gets drunk after eating fermented fruit. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 320 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | elegant glutton |
| **最終更新** | 2019-04-16 23:11:57 Poor Traveler |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Elegant Gluttony](Quests/Adventure/quest_6287.md)** | Adventure quest (4) | Ecological research (3) Biology (5) Oceanian languages | Reward: 145,000/Advance: 10,000Experience: 180 |