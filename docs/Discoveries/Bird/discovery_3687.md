---
id: 3687
category: Bird
---
# Discovery: african lily

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Bird |
| **Discovery Name** | african lily |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A bird native to Africa. The name comes from the fact that they build soft, bag-shaped nests in trees. To deceive predators, they create a false entrance that looks more authentic than the real one, separate from the real entrance. This is the wisdom of a bird that has many natural enemies. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 660 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-08-08 18:46:26 African |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Little bird building a nest](Quests/Adventure/quest_6632.md)** | Adventure quest (6) | Ecological research (6) Biology (8) West African languages ​​(1) | Reward: 129,000 / Advance payment: 10,000 Experience: 170 / Fame: 90 |