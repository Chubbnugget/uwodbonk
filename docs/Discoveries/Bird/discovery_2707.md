---
id: 2707
category: Bird
---
# Discovery: cape vulture

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Bird |
| **Discovery Name** | cape vulture |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A large bird that lives in southern Africa. They build nests on cliffs by combining tree branches. Because there is little terrain suitable for nesting, nests may be concentrated in the same place, or nests may continue to be built in the same place for several generations. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 680 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2016-10-30 11:28:35 Choidru |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Vultures of southern Africa](docs/Quests/Adventure/5001-6000/quest_5293.md)** | Adventure quest (5) | Ecological research (5) Biology (7) West African languages | Reward: 219,000 / Advance payment: 20,000 Experience: 220 / Fame: 130 |