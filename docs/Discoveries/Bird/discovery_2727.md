---
id: 2727
category: Bird
---
# Discovery: shoebill stork

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Bird |
| **Discovery Name** | shoebill stork |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A large bird that lives in wetlands from central to southern Africa, and is characterized by its huge beak. It moves slowly and often stops moving like a statue. It is thought that this is to avoid being alarmed by the fish that are their prey. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 540 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-05-05 18:12:09 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Immobile Bird](Quests/Adventure/quest_5315.md)** | Adventure quest (6) | Ecological research (6) Biology (8) West African languages | Reward: 245,000/ Advance: 30,000Experience: 240/ Fame: 140 |