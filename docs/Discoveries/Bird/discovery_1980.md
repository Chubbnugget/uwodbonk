---
id: 1980
category: Bird
---
# Discovery: More

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Bird |
| **Discovery Name** | More |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A flightless bird that lives in the forests of Aotearoa. It looks similar to an ostrich, but its wings are missing. Some of them are so gigantic that they can be up to 4 meters long, making it hard to believe they are birds. It seems like there used to be a lot of them, but since people started living on the island, the number has decreased dramatically. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 15 (Rank of skills required for discovery) |
| **Experience** | 1222 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Monster bird again |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Monster Bird Again](docs/Quests/Adventure/1001-2000/quest_1722.md)** | Adventure quest (10) | Ecological research (13) Biology (15) Oceanian languages | Reward: 590,000 / Advance payment: 50,000 Experience: 950 / Fame: 740 |