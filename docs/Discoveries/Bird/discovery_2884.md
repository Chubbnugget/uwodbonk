---
id: 2884
category: Bird
---
# Discovery: hawaiian goose

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Bird |
| **Discovery Name** | hawaiian goose |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A bird that lives on the islands near Hawaii. Grows to a total length of around 60cm. The locals call it ``Nene'' because of its cry. |
| **Era** |  |
| **Deck Points** | 2 (Required Deck Points for debate deck) |
| **Difficulty** | 4 (Rank of skills required for discovery) |
| **Experience** | 140 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | hawaii bird map 



 Discovery Experience 140 Discovery Name Card Experience 70 Cultural Contribution 20 |
| **最終更新** | 2022-08-26 13:55:38 Maeda |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Hawaiian bird map](Quests/Other/quest_5559.md)** | Biology学(4) | Ecological Survey(4)Biology(4) | Experience：140 |