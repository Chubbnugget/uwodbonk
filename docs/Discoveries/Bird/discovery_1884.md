---
id: 1884
category: Bird
---
# Discovery: blue jay

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Bird |
| **Discovery Name** | blue jay |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A bird that lives in eastern North America. The top of the head, back, and tail feathers are covered with blue feathers. There is something called a crown feather on the top of the head, which gives it a brave appearance. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 330 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | high-pitched blue bird |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[High-pitched blue bird](docs/Quests/Adventure/2001-3000/quest_2954.md)** | Adventure quest (1) | Ecological research (3) Biology (5) Maya languages ​​(1) | Reward: 87,000 / Advance payment: 0Experience: 150 / Fame: 70 |