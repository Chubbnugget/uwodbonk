---
id: 1981
category: Bird
---
# Discovery: Galah

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Bird |
| **Discovery Name** | Galah |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A bird that lives in flocks throughout Australia. The body is pink, the wings are gray, and the tail is black. It feeds on fruits, insects, etc. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 280 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | pink bird

 Discovery Experience 280 Discovery Name Card Experience 140 |
| **最終更新** | 2016-06-10 21:07:15 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Pink bird](docs/Quests/Adventure/1001-2000/quest_1626.md)** | Adventure quest (4) | Ecological Survey (3) Biology (5) Malayu Tagalog | Reward: 69,000 / Advance payment: 0Experience: 260 / Fame: 170 |