---
id: 1988
category: Bird
---
# Discovery: Gray parrot

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Bird |
| **Discovery Name** | Gray parrot |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | It is a plain parakeet with a gray body and a red tip on its tail. However, they appear to be very intelligent, and if taught well, they can have simple conversations rather than imitation. The way it flies is confusing. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 462 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | bird that speaks human language |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Bird that speaks human language](Quests/Adventure/quest_704.md)** | Adventure quest (7) | Ecological Survey (5) Biology (7) Portuguese | Reward: 69,200/ Advance payment: 10,000 Experience: 130/ Fame: 65 |