---
id: 3285
category: Bird
---
# Discovery: Rainbow pheasant

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Bird |
| **Discovery Name** | Rainbow pheasant |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A large pheasant that lives in India and other countries. Compared to other pheasants, it has a good physique and short tail feathers. The male's plumage in particular has a metallic luster reminiscent of a rainbow, which is where the name comes from. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | 1190 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2018-04-22 14:24:06 digiharuka |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Rainbow on the snowy mountain](docs/Quests/Adventure/6001-7000/quest_6118.md)** | Adventure quest (7) | Ecological research (8) Biology (10) Indian languages | Reward: 237,000 / Advance payment: 20,000 Experience: 240 / Fame: 135 |