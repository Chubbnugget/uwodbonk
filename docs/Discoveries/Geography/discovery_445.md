---
id: 445
category: Geography
---
# Discovery: mekong river

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | mekong river |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A large river that flows near Yardein. The water source seems to be deep in the mountains in the far north, but it's an incredibly long river, so investigating it will be difficult. The water depth is generally shallow and the river is not suitable for swimming upstream. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 462 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | A river where mysterious things lurk |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[River where mysterious things lurk](Quests/Adventure/quest_1322.md)** | Adventure quest (6) | Geography (7) Visual recognition (5) Thai-Burmese | Reward: 60,000/ Advance: 10,000Experience: 290/ Fame: 92 |