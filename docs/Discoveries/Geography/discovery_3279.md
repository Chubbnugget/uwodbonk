---
id: 3279
category: Geography
---
# Discovery: cape hahorn

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | cape hahorn |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A cape located at the easternmost tip of Africa. It has prospered as a trading center since ancient times, and such a description can be found in the ``Erythra Sea Guide.'' Ancient ruins remain, and relics are sometimes found. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 390 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Literature research”

 Discovery Name Card:195 |
| **最終更新** | 2022-12-31 13:18:19 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Literature survey](Quests/Adventure/quest_6125.md)** | Adventure quest (6) | Geography (6) Visual recognition (4) Arabic | Reward: 215,000 / Advance payment: 20,000 Experience: 390 / Fame: 125 |