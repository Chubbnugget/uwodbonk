---
id: 421
category: Geography
---
# Discovery: pohnpei island

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | pohnpei island |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A small island that means "on top of a pile of stones." The island is covered with mangroves, and a mountain rises in the center of the island.

 Deep in the forest, you can see rocks that look like buildings. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 470 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest "Undiscovered Island"

 Discovery Name Card:235 |
| **最終更新** | 2022-10-27 23:40:03 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Undiscovered islet](docs/Quests/Adventure/2001-3000/quest_2253.md)** | Adventure Quest (4) 16th Century 5th Period | Geography (7) Visual recognition (5) Oceanian languages | Reward: 207,000 / Advance payment: 20,000 Experience: 250 / Fame: 110 |