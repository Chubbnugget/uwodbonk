---
id: 3588
category: Geography
---
# Discovery: moeraki boulders

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | moeraki boulders |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A huge spherical rock on the Moeraki coast in New Zealand's South Island. There are tortoiseshell-shaped cracks on the surface. According to Maori legend, it is a modified gourd-shaped eel basket that washed ashore from the large canoe Araiteuru, which ran aground on the coast. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2023-09-19 22:18:24 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of huge round rock](docs/Quests/Other/6001-7000/quest_6516.md)** | Pursuit quest (4) | Geography (14) Visual recognition (12) Chinese | Reward: 420,000 / Advance payment: 0Experience: 864 |