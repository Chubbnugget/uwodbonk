---
id: 465
category: Geography
---
# Discovery: Rapa Nui

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | Rapa Nui |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A solitary island located on the eastern side of the Pacific Ocean. There is no doubt that this is the ``Stars Viewing Island'' as there is a huge statue standing on the coast. Rapa Nui means "big island" in the local language. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 10 (Rank of skills required for discovery) |
| **Experience** | 949 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Island to see Stars |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Island to see Stars](docs/Quests/Adventure/1001-2000/quest_1647.md)** | Adventure quest (10) | Geography (10) Visual recognition (8) Dutch | Reward: 208,000 / Advance payment: 20,000 Experience: 450 / Fame: 165 |