---
id: 3589
category: Geography
---
# Discovery: mauritius island

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | mauritius island |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | The largest island in the Mauritius archipelago, located east of Madagascar. Surrounded by coral reefs and white sand, the sea near the coast appears to be a variety of colors. Beautiful sights can be seen even in the inland areas, with places where the soil shines in seven different colors. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 1800 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2021-03-22 03:26:42 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional survey east of Madagascar](docs/Quests/Other/6001-7000/quest_6505.md)** | Pursuit quest (4) | Geography (14) Visualization (12) Swahili (1) | Reward: 310,000 / Advance payment: 0Experience: 480 / Fame: 255 |