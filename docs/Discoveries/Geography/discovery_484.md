---
id: 484
category: Geography
---
# Discovery: delta state

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | delta state |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | The shallow part of an estuary that is formed when sand and mud washed away from upstream are deposited near the estuary. If mud continues to accumulate over many years, the shallow waters may eventually become an island. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 624 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | The island that splits the taiga |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[The island that splits the great river](docs/Quests/Adventure/1001-2000/quest_1258.md)** | Adventure quest (8) 16th century 4th period | Geography (8) Visual recognition (6) Dutch | Reward: 250,000 / Advance payment: 20,000 Experience: 330 / Fame: 120 |