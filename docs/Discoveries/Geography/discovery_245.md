---
id: 245
category: Geography
---
# Discovery: galapagos islands

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | galapagos islands |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A volcanic archipelago consisting of multiple islands located directly below the equator. It seems like there are many animals that can't be seen anywhere else. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 9 (Rank of skills required for discovery) |
| **Experience** | 750 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Investigating the island you wandered into”

 Discovery Name Card:375 |
| **最終更新** | 2022-11-11 21:23:48 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Investigating the island you wandered into](docs/Quests/Adventure/2001-3000/quest_2239.md)** | Adventure Quest (7) 16th Century 2nd Period | Geography (9) Visualization (7) Quechua | Reward: 212,000 / Advance payment: 20,000 Experience: 250 / Fame: 110 |