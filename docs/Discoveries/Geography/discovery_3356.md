---
id: 3356
category: Geography
---
# Discovery: Ghaggar Harkra River

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | Ghaggar Harkra River |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A river that is said to have flowed parallel to the Indus River in ancient times. Although it only flows during the rainy season now, there is a hymn to it in the Rig Veda as the Sarasvati River. One theory is that the Hindu goddess Sarasvati is the deification of this river. |
| **Era** |  |
| **Deck Points** | 10 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1030 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2018-10-20 16:55:18 digiharuka |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[The basis there](docs/Quests/Adventure/6001-7000/quest_6222.md)** | Adventure Quest (8) B.C. | Geography (12) Religious Studies (8) Visualization (10) | Reward: 164,000 / Advance payment: 10,000 Experience: 190 / Fame: 105 |