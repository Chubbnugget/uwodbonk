---
id: 463
category: Geography
---
# Discovery: Rakkadjib Islands

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | Rakkadjib Islands |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | Islands floating to the west of southern India. It consists of nearly 40 islands. It seems that a small number of people live there, making a living by cultivating palm trees and fishing. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 320 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest "Pirate Hangout"

 Discovery Name Card:160 |
| **最終更新** | 2022-10-14 14:01:43 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Pirate Hangout](docs/Quests/Adventure/1-1000/quest_380.md)** | Adventure quest (6) | Geography (6) Visual recognition (4) | Reward: 99,000 / Advance payment: 0Experience: 150 / Fame: 85 |