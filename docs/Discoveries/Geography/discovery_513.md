---
id: 513
category: Geography
---
# Discovery: 白馬江

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | 白馬江 |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A river that flows through the land where Baekje and Silla once fought. The wide river width, gentle flow, white sandbar, and green mountain range harmonize to create a beautiful landscape. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 460 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | river of history |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[River of History](docs/Quests/Adventure/2001-3000/quest_2674.md)** | Adventure quest (4) | Geography (6) Visual recognition (4) Korean (1) | Reward: 72,000 / Advance payment: 0Experience: 130 / Fame: 65 |