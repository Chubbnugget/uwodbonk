---
id: 246
category: Geography
---
# Discovery: ganges river

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | ganges river |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A large river that flows from eastern India to the Bay of Bengal. It is considered a sacred river for Hindus. The basin is covered with extremely fertile land. It seems that civilization has developed since ancient times. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 270 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest "Sacred Rivers of India" 

 Discovery Name Card:135 |
| **最終更新** | 2022-11-14 20:39:59 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Sacred rivers of India](Quests/Adventure/quest_66.md)** | Adventure Quest (6) Mythology Era | Geography (5) Visual recognition (3) Indian languages | Reward: 73,000Experience: 160/ Fame: 75 |