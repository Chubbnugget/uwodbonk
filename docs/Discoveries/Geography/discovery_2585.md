---
id: 2585
category: Geography
---
# Discovery: olympus mount

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | olympus mount |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | The highest mountain in Greece, located southwest of Salonica. It is the highest mountain in Europe, with its base close to the sea. In Greek mythology, it is said to be the mountain where the gods known as the Twelve Olympus dwell. |
| **Era** |  |
| **Deck Points** | 2 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 140 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2018-02-20 22:18:31 China's Fei-Kun @Hei-Kun |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Let's search for God's mountain](docs/Quests/Adventure/4001-5000/quest_4950.md)** | Adventure quest (2) | Geography (2) Visualization (1) Greek | Reward: 72,000Experience: 130/ Fame: 65 |