---
id: 3206
category: Geography
---
# Discovery: Burgess Shale

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | Burgess Shale |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | One of the sedimentary layers in the western part of the North American continent. A number of strange stones have been discovered that appear to be bones of some kind of Biology. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | 9 (Rank of skills required for discovery) |
| **Experience** | 950 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2017-11-21 22:54:54 Again |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Treasure trove of strange stones](docs/Quests/Adventure/6001-7000/quest_6018.md)** | Adventure quest (9) | Geography (11) Visual recognition (9) North American languages | Reward: 514,000 / Advance payment: 50,000 Experience: 410 / Fame: 250 |