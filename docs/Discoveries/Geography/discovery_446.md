---
id: 446
category: Geography
---
# Discovery: mesa

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | mesa |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A plateau shaped like a table. Common in western North America. If there is a horizontal hard stratum on top of a soft stratum, it is likely that only the lower soft stratum has eroded, creating a table-like shape. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 610 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Assistance to researchers |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Assistance to researchers](Quests/Adventure/quest_3298.md)** | Adventure quest (5) | Geography (7) Visual recognition (5) Spanish | Reward: 199,000 / Advance payment: 10,000 Experience: 240 / Fame: 105 |