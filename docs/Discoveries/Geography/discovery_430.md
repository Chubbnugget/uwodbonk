---
id: 430
category: Geography
---
# Discovery: Straits of Malacca

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | Straits of Malacca |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A strait between the Malay Peninsula and Sumatra. Although it is an important transit point for east-west trade, the strait is narrow and the water depth is extremely shallow. Furthermore, there are many reefs and shallow waters, so extreme caution is required when navigating. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 320 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Imperial command quest “[Important point for advancing to the Spice Islands”]

Imperial command alternative quest “The narrow road leading to the eastern country”

 Discovery Name Card:160 |
| **最終更新** | 2022-11-05 18:20:32 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Narrow road leading to eastern country](Quests/Adventure/quest_1331.md)** | Adventure quest (5) 16th century 1st period | Geography (6) Visual recognition (4) Malayu Tagalog | Reward: 70,000 / Advance payment: 5,000 Experience: 250 / Fame: 82 |
| **[Key point for advancing towards the Spice Islands](Quests/Other/quest_2075.md)** | Imperial command quest (3) | Geography (6) Visual recognition (4) | Experience: 250/ Reputation: 110 |