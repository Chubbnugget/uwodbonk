---
id: 198
category: Geography
---
# Discovery: Atacama Salt Lake

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | Atacama Salt Lake |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A vast salt lake in the Atacama Desert. The rocks on the lake shore are dyed white with salt, making it look like snow has piled up. This salt lake is also a resting place for flamingos. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 620 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | A place where birds gather |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[A place where birds gather](Quests/Adventure/quest_1773.md)** | Adventure quest (6) | Geography (8) Visualization (6) Quechua | Reward: 112,000 / Advance payment: 10,000 Experience: 170 / Fame: 80 |