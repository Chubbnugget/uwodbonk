---
id: 326
category: Geography
---
# Discovery: Darien Bay

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Geography |
| **Discovery Name** | Darien Bay |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A bay in the southern Caribbean Sea. Admiral Colon visited, and a port was subsequently built by the Spanish people. It is known as a good natural port. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 4 (Rank of skills required for discovery) |
| **Experience** | 120 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Road to the West Coast of the New World”

 Discovery Name Card:60 |
| **最終更新** | 2022-12-06 21:10:17 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Road to the west coast of the New World](Quests/Adventure/quest_1210.md)** | Adventure Quest (4) 15th Century 5th Period | Geography (4) Visual recognition (2) Spanish | Reward: 50,000/ Advance payment: 1,000 Experience: 165/ Fame: 65 |