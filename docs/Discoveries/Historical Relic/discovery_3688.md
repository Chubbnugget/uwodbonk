---
id: 3688
category: Historical Relics
---
# Discovery: silver pepper shaker

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | silver pepper shaker |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | Pepper shaker from the Roman Empire. It is shaped like a lady. At that time, pepper was a valuable item, and pepper containers were also decorated with gold leaf. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 1500 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Discovery Name Card 750 |
| **最終更新** | 2022-08-07 02:33:07 Sakizo @larksjp |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Silver pepper shaker](docs/Quests/Other/6001-7000/quest_6635.md)** | Legacy(1) | - |  |