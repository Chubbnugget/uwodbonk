---
id: 2557
category: Historical Relic
---
# Discovery: El Dorado (Shining Hill)

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relic |
| **Discovery Name** | El Dorado (Shining Hill) |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | A temple full of gold and jewels hidden deep in the jungles of the New World. It is difficult to visit as it is kept a secret by the residents. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2018-10-24 23:29:39 Arata Kashiwaba |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[The Endless Land of Gold](Quests/Other/quest_3537.md)** | Treasure hunting (10) | Religious Studies(13)Exploration(15)Treasures Identification(17) | Experience: 3,100/ Reputation: 2,480 |