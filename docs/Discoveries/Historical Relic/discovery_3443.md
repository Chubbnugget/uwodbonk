---
id: 3443
category: Historical Relic
---
# Discovery: scorching large lantern

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relic |
| **Discovery Name** | scorching large lantern |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | A huge fiery vessel held by the Colossus of Rhodes. It is said that blazing flames were poured down from this vessel on ships that entered the port illegally. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Treasure Hunt “Lores related to old maps” |
| **最終更新** | 2019-07-28 22:16:00 Koumeitan |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Legend related to the Colossus](docs/Quests/Other/6001-7000/quest_6354.md)** | Treasure hunting (7) | Archeology (13) Visualization (11) Greek (1) | Remuneration: 0/Advance payment: 0 |