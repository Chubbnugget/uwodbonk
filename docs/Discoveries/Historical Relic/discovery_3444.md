---
id: 3444
category: Historical Relic
---
# Discovery: ancient reflector

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relic |
| **Discovery Name** | ancient reflector |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | It is said that a reflector placed near the top of the Great Lighthouse of Alexandria focused and reflected the sun's rays, making it possible to burn ships far away. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Treasure Hunt “Lores related to old maps” |
| **最終更新** | 2019-07-28 23:51:08 Koumeitan |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Legend related to the Great Lighthouse](Quests/Other/quest_6355.md)** | Treasure hunting (7) | Archeology (13) Visualization (11) Arabic (1) | Reward: 0/Advance payment: 0Experience: 2,410 |