---
id: 1076
category: Historical Relics
---
# Discovery: Alexander the Great's Armor

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | Alexander the Great's Armor |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | The armor of Alexander the Great, the Macedonian king who built a huge empire from the Mediterranean to Persia. It has a luxurious structure using Egyptian and Persian jewels. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 506 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | The land where the king returns |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Land of the King's Return](docs/Quests/Adventure/1-1000/quest_360.md)** | Adventure Quest (7) B.C. | Exploration (5) Archeology (7) Unlocking (5) | Reward: 2,000 Experience: 320 / Fame: 135 |