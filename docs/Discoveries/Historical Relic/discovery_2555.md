---
id: 2555
category: Historical Relic
---
# Discovery: Another Jeanne

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relic |
| **Discovery Name** | Another Jeanne |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | It is said that the brave girl who stole the Burgundian flag and saved Beauvais was named Jeanne, the same name as La Pucelle. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Obtain Jeanne Hachette's ax |
| **最終更新** | 2018-10-17 01:46:56 Arata Kashiwaba |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Another Jeanne](docs/Quests/Other/3001-4000/quest_3541.md)** | Treasure hunting (8) | Exploration (12) Archeology (14) French | Experience：3,145 |