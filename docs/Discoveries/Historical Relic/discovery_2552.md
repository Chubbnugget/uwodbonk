---
id: 2552
category: Historical Relic
---
# Discovery: Knights Templar

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relic |
| **Discovery Name** | Knights Templar |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | An order of knights that was active in the Middle Ages and had a huge amount of assets. There is a legend that he brought back some secret treasures from the sacred place, but the truth remains in the darkness. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2018-10-16 17:27:22 Arata Kashiwaba |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Sacred Lands, Knights, and Treasures](Quests/Other/quest_3427.md)** | Treasure hunting (7) | Religion(12)Exploration(10)TreasuresIdentification(8) | Experience: 2,270/ Reputation: 905 |