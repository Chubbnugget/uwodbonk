---
id: 3903
category: Historical Relic
---
# Discovery: A road leading to an ancient trading city

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relic |
| **Discovery Name** | A road leading to an ancient trading city |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | In southern China, there was a city called Chang'an that was extremely prosperous due to East-West trade. It is said that merchants from various countries used to come and go on the road leading there. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2024-10-16 11:40:07 Nicolette |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[A prosperous capital since ancient times](docs/Quests/Other/6001-7000/quest_6891.md)** | Treasure hunting (9) | Exploration(13)Archaeology(15)Chinese(1) |  |