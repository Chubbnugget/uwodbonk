---
id: 2545
category: Historical Relic
---
# Discovery: Numa Pompilius

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relic |
| **Discovery Name** | Numa Pompilius |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | The Roman Empire ruled the ancient Mediterranean world. The person who laid the foundation for this was surprisingly a wise king invited from outside Rome. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2018-11-21 12:58:55 Arata Kashiwaba |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Roman Philosopher King](Quests/Other/quest_3531.md)** | Treasure hunting (4) | Religious Studies (8) Exploration (6) Latin | Experience：1,580 |