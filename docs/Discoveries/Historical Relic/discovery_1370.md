---
id: 1370
category: Historical Relics
---
# Discovery: Vertical eye mask

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | Vertical eye mask |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A uniquely shaped mask.

 It has distinctive vertical eyes, and its shape is reminiscent of the legendary first king of Shu, Silkworm. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1060 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Sericulture and silkworm flora

 Discovery Experience 1060 Discovery Name Card Experience 530 |
| **最終更新** | 2016-07-25 13:32:30 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Sericulture and silkworm flora](docs/Quests/Adventure/2001-3000/quest_2851.md)** | Adventure quest (9) | Exploration (10) Archeology (12) Unlocking (10) | Reward: 280,000 / Advance payment: 20,000 Experience: 310 / Fame: 260 |