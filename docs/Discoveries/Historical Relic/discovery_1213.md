---
id: 1213
category: Historical Relics
---
# Discovery: Kublai Khan's saddle

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | Kublai Khan's saddle |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A horse saddle said to have been used by Yuan Dynasty emperor Kublai. Mongolians, a cavalry people, generally use wooden saddles, which are made of tanned leather for long marches. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 290 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | footsteps of a great empire |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Footsteps of the Great Empire](docs/Quests/Adventure/1001-2000/quest_1278.md)** | Adventure quest (4) | Exploration (3) Archeology (5) Unlocking (3) | Reward: 63,000 / Advance payment: 0Experience: 220 / Fame: 70 |