---
id: 2569
category: Historical Relic
---
# Discovery: Karl Martel

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relic |
| **Discovery Name** | Karl Martel |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | What would have happened to European history if Karl Martel had lost the battle between Tours and Poitiers? |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2018-10-17 01:53:32 Arata Kashiwaba |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Warhammer of Karl Martel](Quests/Other/quest_3545.md)** | Treasure hunting (6) | Exploration (9) Archeology (11) French |  |