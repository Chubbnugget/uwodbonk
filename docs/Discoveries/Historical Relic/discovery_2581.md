---
id: 2581
category: Historical Relic
---
# Discovery: Thief of Sherwood Forest

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relic |
| **Discovery Name** | Thief of Sherwood Forest |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | Robin Hood is a polite outlaw who is said to have lived in Sherwood Forest. It has been passed down from generation to generation and continues to be loved by people. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 3030 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Robber's Bow

Experience 3030 when discovered Fame 2420 when discovered

 (It is set to 1210*2 because the reputation will be halved due to the total fame exceeding 20,000) |
| **最終更新** | 2018-10-17 00:20:50 Arata Kashiwaba |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Bow of the Robber](Quests/Other/quest_3704.md)** | Treasure hunting (9) | 探索(13)Treasures鑑定(15)英語 | Reward: 0 / Advance payment: 0 Experience: 3,030 / Fame: 2,420 |