---
id: 3544
category: Historical Relics
---
# Discovery: ancient silvered glassware

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relics |
| **Discovery Name** | ancient silvered glassware |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | An ancient glassware with a colorful rainbow-like pattern. Glass that has spent many years in sand or soil undergoes chemical changes in its components such as silicic acid and aluminum oxide due to the surrounding materials, causing it to shine like metal. This is called the silvering phenomenon. |
| **Era** |  |
| **Deck Points** | 8 (Required Deck Points for debate deck) |
| **Difficulty** | 3 (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-12-25 00:01:22 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Additional investigation of ancient glass](docs/Quests/Other/6001-7000/quest_6466.md)** | Pursuit quest (3) | Exploration (8) Archeology (10) Italian (1) | Reward: 190,000 / Advance payment: 0Experience: 1,100 |