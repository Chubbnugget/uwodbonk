---
id: 2578
category: Historical Relic
---
# Discovery: Preste Joan

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Historical Relic |
| **Discovery Name** | Preste Joan |
| **Rank** | (Number of Stars on Discovery Name card) |
| **explanation** | It is said that there were many Christians among the Kerait tribe led by Ong Han. That's probably why it became part of the legend of Preste João. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 3050 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-11-14 10:26:15 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Ong Han's Armor](docs/Quests/Other/3001-4000/quest_3547.md)** | Treasure hunting (10) | Exploration (12) Archeology (14) Indian languages | Experience: 3,050/ Reputation: 714 |