---
id: 637
category: Artwork
---
# Discovery: saint jerome and the lion

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Artwork |
| **Discovery Name** | saint jerome and the lion |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | Bronze statue by Bartolomeo Verano. St. Jerome is shown pulling out a thorn from a lion's paw. Bronze statues can usually be reproduced using a mold, but this work is a one-of-a-kind piece made from a wax mold, and there are no copies available. |
| **Era** |  |
| **Deck Points** | 2 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 300 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | bronze statue of the saint and the beast |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Bronze statue of the saint and the beast](Quests/Adventure/quest_732.md)** | Adventure Quest (6) 15th Century 5th Period | Exploration (3) Art (5) Italian | Reward: 50,000 / Advance payment: 0Experience: 160 / Fame: 75 |