---
id: 2602
category: Artwork
---
# Discovery: Statue of Theseus and the Minotaur

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Artwork |
| **Discovery Name** | Statue of Theseus and the Minotaur |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A statue depicting the hero Theseus from Greek mythology fighting the Minotaur. In order to save Athens from the crisis, Theseus went to the labyrinth in the palace of Knossos on the island of Crete and defeated the Minotaur. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 740 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2016-11-14 22:05:21 Choidru |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[The story of the cow ghost](docs/Quests/Adventure/4001-5000/quest_4974.md)** | Adventure Quest (6) Mythology Era | Exploration (6) Art (8) Greek | Reward: 208,000 / Advance payment: 20,000 Experience: 250 / Fame: 110 |