---
id: 629
category: Artwork
---
# Discovery: Wang Meng's landscape paintings

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Artwork |
| **Discovery Name** | Wang Meng's landscape paintings |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A painting by Wang Meng, a painter from the Yuan Dynasty. The undulating mountains are depicted in a detailed and magnificent manner that overwhelms the viewer. |
| **Era** |  |
| **Deck Points** | 5 (Required Deck Points for debate deck) |
| **Difficulty** | 5 (Rank of skills required for discovery) |
| **Experience** | 350 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Disciple of Huang Gongwang |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Disciple of Huang Gongwang](docs/Quests/Adventure/2001-3000/quest_2872.md)** | Error 500 (Server Error)!!1500.That’s an error.There was an error. Please try again later.That’s all we know. | 探索(3)美術(5)中国語(1) | Reward: 98,000 / Advance payment: 0Experience: 160 / Fame: 75 |