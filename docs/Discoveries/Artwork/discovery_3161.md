---
id: 3161
category: Artwork
---
# Discovery: islamic carving box

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Artwork |
| **Discovery Name** | islamic carving box |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | Ivory work made during the Later Umayyad Dynasty. Separate scenes are depicted on each of the four sides in elaborate relief, and detailed patterns are engraved to connect the four sides. It looks like it was made for a noble person. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 1370 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2022-10-22 13:56:01 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Islamic artefacts](Quests/Adventure/quest_5844.md)** | Adventure quest (10) | Exploration (15) Art (17) Arabic | Reward: 924,000 / Advance payment: 90,000 Experience: 660 |