---
id: 654
category: Artwork
---
# Discovery: ivory flute

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Artwork |
| **Discovery Name** | ivory flute |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | An ivory flute made in Africa. It has a simple and unique carving that takes advantage of the beautiful curves inherent in ivory. It is thought that it was used for practical purposes rather than for decoration. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 360 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | indian elephant tamer |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Indian Elephant Tamer](docs/Quests/Adventure/1-1000/quest_59.md)** | Adventure quest (7) | Exploration (4) Art (6) Indian languages | Reward: 102,000 / Advance payment: 0Experience: 260 |