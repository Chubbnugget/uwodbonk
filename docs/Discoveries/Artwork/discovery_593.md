---
id: 593
category: Artwork
---
# Discovery: Annunciation (Martini)

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Artwork |
| **Discovery Name** | Annunciation (Martini) |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | Altarpiece by Simone Martini. At the time of its creation, it was a large-scale work consisting of multiple paintings. As a religious painting from the Era, the figures are expressive and suggest the arrival of a new Era. |
| **Era** |  |
| **Deck Points** | 1 (Required Deck Points for debate deck) |
| **Difficulty** | 1 (Rank of skills required for discovery) |
| **Experience** | 40 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | church religious painting map |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Church religious painting map](docs/Quests/Other/1-1000/quest_450.md)** | 美術(1) | 探索(1)美術(1) |  |