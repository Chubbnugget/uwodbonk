---
id: 2843
category: Artwork
---
# Discovery: Hand-woven basket with pattern

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Artwork |
| **Discovery Name** | Hand-woven basket with pattern |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | Hand-woven baskets made in West Africa. It is woven with brightly colored patterns and has artistic value. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 470 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Beautiful baskets from West Africa

 Discovery Experience 470 Discovery Name Card Experience 235 |
| **最終更新** | 2016-09-20 21:37:47 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Beautiful basket from West Africa](Quests/Adventure/quest_5513.md)** | Adventure quest (6) | Exploration (5) Art (7) West African languages | Reward: 161,000 / Advance payment: 10,000 Experience: 190 / Fame: 105 |