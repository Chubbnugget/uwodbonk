---
id: 2957
category: WeatherPhenomena
---
# Discovery: water tornado

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | WeatherPhenomena |
| **Discovery Name** | water tornado |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A tornado that occurs at sea. In addition to strong gusts of wind, the erratic movement of water causes ships to become engulfed and sink in rare cases. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 519 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | The western tip of the island, west of Las Palmas. 

 It seems to be limited to cloudy days. |
| **最終更新** | 2017-05-09 21:53:53 Sakura @ Guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Water Tornado](Quests/Other/quest_5639.md)** | Weather/phenomena (3) | Visual recognition (6) | Experience：470 |