---
id: 3006
category: WeatherPhenomena
---
# Discovery: lenticular cloud

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | WeatherPhenomena |
| **Discovery Name** | lenticular cloud |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | Flat-shaped clouds that appear above mountains and mountain ranges. It is known in many places as a harbinger of bad weather, as it tends to occur when there is strong wind in the sky. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 370 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | 2017.02.07 New addition |
| **最終更新** | 2022-05-27 10:43:38 Maeda |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Lens cloud](Quests/Other/quest_5717.md)** | Weather/phenomena (1) | Visual recognition (6) |  |