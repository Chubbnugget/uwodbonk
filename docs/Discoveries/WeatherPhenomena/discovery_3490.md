---
id: 3490
category: WeatherPhenomena
---
# Discovery: arch clouds

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | WeatherPhenomena |
| **Discovery Name** | arch clouds |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A phenomenon in which clouds form an arch. They form where cold air and warm air from cumulonimbus clouds collide. When arch clouds form, strong wind gusts and thunderstorms often occur. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 320 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2020-05-10 21:27:01 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Arch cloud](Quests/Other/quest_6410.md)** | Weather/phenomena (1) | Visual recognition (10) | Experience：320 |