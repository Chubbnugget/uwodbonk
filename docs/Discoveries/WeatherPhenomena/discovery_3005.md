---
id: 3005
category: WeatherPhenomena
---
# Discovery: sand tornado

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | WeatherPhenomena |
| **Discovery Name** | sand tornado |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A tornado that occurs in a desert area. Because it throws up a lot of sand, it looks different from a normal tornado. A large sand tornado is said to be large enough to swallow a small town. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Deeper from the northern coast of Africa, near the diagonal rocks of the Sahara 

 during the day |
| **最終更新** | 2017-02-09 01:45:10 Atton |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Sand Tornado](Quests/Other/quest_5715.md)** | Weather/phenomena (2) | Visual recognition (5) | Experience：290 |