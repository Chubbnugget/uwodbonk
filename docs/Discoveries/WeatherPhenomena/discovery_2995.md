---
id: 2995
category: WeatherPhenomena
---
# Discovery: monster of brocken

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | WeatherPhenomena |
| **Discovery Name** | monster of brocken |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A phenomenon in which light from the sun or other sources shines in from behind, creating a rainbow-like ring of light around the shadow. To occur, it requires widespread fog and open areas with unobstructed sunlight. Therefore, there are many examples of observations at high altitudes such as mountains. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 780 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | 2017.02.07 New addition

In visibility R9, a message appeared saying "Skill Rank is insufficient to discover." Found in R11.

 I was able to find it with R10. I guess it's confirmed. |
| **最終更新** | 2022-05-24 09:28:21 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Brocken Monster](docs/Quests/Other/5001-6000/quest_5703.md)** | Weather/phenomena (1) | Visual recognition (10) |  |