---
id: 2853
category: WeatherPhenomena
---
# Discovery: circumhorizontal arc

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | WeatherPhenomena |
| **Discovery Name** | circumhorizontal arc |
| **Rank** | ★★★★☆(4) (Number of Stars on Discovery Name card) |
| **explanation** | A phenomenon in which rainbow-colored bands of light appear overlapping thin clouds under the sun. Unlike other optical phenomena, it appears as a horizontal band rather than a circle. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 380 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | East coast of the North American continent (landing point) during the day? Sunny weather? Near the south end of the southeast beach, visibility R9 or higher may be required?

 It seems that Experience is an adventure job and has been registered as 760, which has been corrected. |
| **最終更新** | 2021-04-14 18:50:36 Passing navigator @guest |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[環水平アーク](docs/Quests/Other/5001-6000/quest_5533.md)** | Weather/phenomena (4) | Visual recognition (9) | Experience：760 |