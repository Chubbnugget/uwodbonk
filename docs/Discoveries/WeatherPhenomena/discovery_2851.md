---
id: 2851
category: WeatherPhenomena
---
# Discovery: Haboob

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | WeatherPhenomena |
| **Discovery Name** | Haboob |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A huge sandstorm that occurs in the desert areas of northern Africa. At times, it can become large enough to swallow an entire city. They move quickly, and when discovered, prompt evacuation indoors is required. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 220 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | A phenomenon in which sandstorms occur on the other side of the Sahara river.

 Places along rivers do not experience sandstorms. |
| **最終更新** | 2016-04-22 09:26:50 Gone Napo |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Haboob](Quests/Other/quest_5528.md)** | Weather/phenomena (3) | Visual recognition (6) | Experience：440 |