---
id: 3531
category: WeatherPhenomena
---
# Discovery: light pillar

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | WeatherPhenomena |
| **Discovery Name** | light pillar |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A phenomenon in which a pillar of light appears to extend from the ground toward the sky. In cold regions, they appear like pillars when light from far away ground reflects off thin clouds containing ice crystals floating in the sky. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 260 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | In front of Stockholm port. night. Probably winter (observed on the night just after winter)

 Discovery Name Card:130 |
| **最終更新** | 2023-03-20 11:07:13 Maeda |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Light Pillar](docs/Quests/Other/6001-7000/quest_6453.md)** | Weather/phenomena (2) | Visual recognition | Experience：260 |