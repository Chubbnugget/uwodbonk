---
id: 3045
category: Marine Life
---
# Discovery: Ohmri

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Marine Life |
| **Discovery Name** | Ohmri |
| **Rank** | ★☆☆☆☆(1) (Number of Stars on Discovery Name card) |
| **explanation** | A salmon-like fish that lives in Lake Baikal. During the spawning season, they swim up rivers that flow into the lake, and return to the lake after spawning. It is very delicious when eaten. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | (Rank of skills required for discovery) |
| **Experience** | 190 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | 2017.02.07 New addition

 Possibly based on the discovery of Golomyanka |
| **最終更新** | 2022-06-01 11:50:36 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Map of fish living in the lake](docs/Quests/Other/5001-6000/quest_5758.md)** | Biology学(6) | Ecological Survey(6)Biology(6) |  |