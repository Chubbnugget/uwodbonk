---
id: 2391
category: Marine Life
---
# Discovery: large marlin

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Marine Life |
| **Discovery Name** | large marlin |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | Marlin is a large pelagic fish, but this individual is exceptionally large. Because it has a sharp snout and swims at high speed, it can be sunk if it collides with a ship. It would be very difficult to catch it. |
| **Era** |  |
| **Deck Points** | 4 (Required Deck Points for debate deck) |
| **Difficulty** | 7 (Rank of skills required for discovery) |
| **Experience** | 484 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Giant fish as big as a boat” |
| **最終更新** | 2022-10-30 16:48:38 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Giant fish as big as a boat](docs/Quests/Adventure/1-1000/quest_776.md)** | Adventure quest (7) | Ecological research (5) Biology (7) West African languages | Reward: 124,000 / Advance payment: 0Experience: 180 / Fame: 95 |