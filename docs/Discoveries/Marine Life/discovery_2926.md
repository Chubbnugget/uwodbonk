---
id: 2926
category: Marine Life
---
# Discovery: Smooth mange crab

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Marine Life |
| **Discovery Name** | Smooth mange crab |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A small crab distributed from the Indian Ocean to the western Pacific Ocean. It is characterized by a smooth and rounded instep. It is poisonous, and its composition and amount vary depending on where it lives. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 530 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Quest “Candy-like crab”

 Discovery Name Card:275 |
| **最終更新** | 2022-12-22 09:12:25 Sakon. |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Candy-like crab](docs/Quests/Adventure/5001-6000/quest_5590.md)** | Adventure quest (6) | Ecological Survey (6) Biology (8) Japanese | Reward: 212,000 / Advance payment: 20,000 Experience: 220 / Fame: 125 |