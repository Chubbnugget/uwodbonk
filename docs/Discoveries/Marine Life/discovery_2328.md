---
id: 2328
category: Marine Life
---
# Discovery: Hawksbill turtle

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Marine Life |
| **Discovery Name** | Hawksbill turtle |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A rather small sea turtle. The upper beak is curved like a hawk. The story goes that shells are used for crafts in the East. |
| **Era** |  |
| **Deck Points** | 3 (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 250 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Map of sea turtles |
| **最終更新** | 2015-01-27 02:09:17 System |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Map of sea turtles](Quests/Other/quest_377.md)** | Biology学(8) | Ecological Survey(8)Biology(8) |  |