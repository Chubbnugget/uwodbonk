---
id: 3175
category: Marine Life
---
# Discovery: Torikajika

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Marine Life |
| **Discovery Name** | Torikajika |
| **Rank** | ★★☆☆☆(2) (Number of Stars on Discovery Name card) |
| **explanation** | A member of the sea bass family that lives in the waters around East Asia. The head is large compared to the body, and the tail is thin and short. The name comes from the large eyes on its large head, which resemble a bird's face. |
| **Era** |  |
| **Deck Points** | (Required Deck Points for debate deck) |
| **Difficulty** | 6 (Rank of skills required for discovery) |
| **Experience** | 370 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Found while fishing. I caught it from the Mamiya Strait to the Sea of ​​Japan.

 Sea of ​​Okhotsk (near 6533,2497) |
| **最終更新** | 2022-08-26 15:56:52 Mifuyu |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Torikajika](docs/Quests/Other/5001-6000/quest_5913.md)** | Fishing (6) | fishing |  |