---
id: 3076
category: Marine Life
---
# Discovery: Senju shrimp

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Marine Life |
| **Discovery Name** | Senju shrimp |
| **Rank** | ★★★☆☆(3) (Number of Stars on Discovery Name card) |
| **explanation** | A small shrimp that lives in the deep sea. The name comes from the fact that all of its legs have pincers, and at first glance, each leg looks like a hand. |
| **Era** |  |
| **Deck Points** | 6 (Required Deck Points for debate deck) |
| **Difficulty** | 8 (Rank of skills required for discovery) |
| **Experience** | 600 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | Fishing Discovery Name

 north of edinburgh |
| **最終更新** | 2017-06-26 11:27:48 Sakai Hidemasa |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Senju Shrimp](Quests/Other/quest_5804.md)** | Fishing (8) | Fishing (8) |  |