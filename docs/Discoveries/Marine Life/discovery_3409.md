---
id: 3409
category: Marine Life
---
# Discovery: Japanese dolphin

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Marine Life |
| **Discovery Name** | Japanese dolphin |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | Dolphins live in the waters of South America and the Indian Ocean. The body color is divided into white and black. It is also called the "panda dolphin" because of its body color. Males have a drop-like pattern on their bellies, while females have round-shaped bellies. |
| **Era** |  |
| **Deck Points** | 9 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1010 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** |  |
| **最終更新** | 2019-07-28 16:58:24 digiharuka |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Friend who loves pandas](docs/Quests/Adventure/6001-7000/quest_6325.md)** | Adventure quest (8) | Ecological research (10) Biology (12) Oceanian languages | Reward: 535,000 / Advance payment: 50,000 Experience: 420 / Fame: 260 |