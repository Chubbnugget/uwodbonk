---
id: 2318
category: Marine Life
---
# Discovery: coelacanth

## Discovery Details
| Attribute | Details |
| :--- | :--- |
| **Discovery Type** | Marine Life |
| **Discovery Name** | coelacanth |
| **Rank** | ★★★★★(5) (Number of Stars on Discovery Name card) |
| **explanation** | A strange fish that is different from other fish. There are scales growing at the base of the fins, the shape of the tail fin has changed, and the swim bladder is filled with fat. It's just a hunch, but I can't help but think it's an incredible existence. |
| **Era** |  |
| **Deck Points** | 7 (Required Deck Points for debate deck) |
| **Difficulty** | 12 (Rank of skills required for discovery) |
| **Experience** | 1124 (For adventure jobs. Excluding company benefits, beginner bonuses, etc.) |
| **Remarks** | living stone fish

 Discovery Experience1124 Discovery Name Card Experience562 |
| **最終更新** | 2016-07-30 23:30:45 Gainor |

---
## Related Quests
The following quests are associated with this discovery:

| Quest Name | Category | Necessary Skills | Rewards |
| :--- | :--- | :--- | :--- |
| **[Living Stone Fish](Quests/Adventure/quest_1984.md)** | Adventure quest (9) | Ecological research (10) Biology (12) West African languages | Reward: 280,000 / Advance payment: 10,000 Experience: 495 / Fame: 165 |